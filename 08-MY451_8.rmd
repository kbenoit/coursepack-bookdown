# Linear regression models {#c_regression}

## Introduction {#s_regression_intro}
...
\@ref(s_regression-descr) first discusses graphical and numerical
...
\@ref(s_regression-simple) then describes methods of statistical
...
analysis briefly in Section \@ref(s_regression-causality). Multiple
linear models are then introduced in Section
\@ref(s_regression-multiple). The models can also include categorical
explanatory variables with any number of categories, as explained in
Section \@ref(s_regression-dummies).

The following example will be used for illustration throughout this
chapter:

**Example 8.1: Indicators of Global Civil Society** \label{p_civilsoc}
...
## Describing association between two continuous variables {#s_regression_descr}

### Introduction {#ss_regression_descr_intro}
...
methods in Section \@ref(ss_regression_descr-plots). This leads to a
discussion in Section \@ref(ss_regression_descr-assoc) of what we
...
\@ref(ss_regression_descr-corr). Statistical *inference* for the
associations will be considered in Section \@ref(s_regression-simple).

### Graphical methods {#ss_regression_descr_plots}
...
in Figure \@ref(f-corruption1), which shows a scatterplot of Control of
...
![A scatterplot of Control of corruption vs. GDP per capita in the
...
corresponding to the United Kingdom.<span
data-label="f_corruption1"></span>](corruption1){width="14cm"}
...
of $X$ and $Y$. For example, Figure \@ref(f-corruption1) highlights the
...
\@ref(s_descr1-presentation)) are the same for scatterplots as for any
...
(as it is for the $X$-axis of Figure \@ref(f-corruption1)). Instead, the
...
\@ref(f-corruption1), for example, it seems that most of the points are
...
\@ref(ss_regression_descr-assoc) below. Before that, however, we will
...
example (another one is Figure \@ref(f-houseprices) on page
\pageref{f_houseprices}):

*Example: Changes in temperature, 1903–2004*

Figure \@ref(f-temperatures) summarises data on average annual
...
![Changes of average annual temperature (11-year moving averages) from
1903 in five locations. See the text for further details.<span
data-label="f_temperatures"></span>](temperplot "fig:"){width="13cm"}\
Source: The GISTEMP database <data.giss.nasa.gov/gistemp/>
...
$Y$-axis. In Figure \@ref(f-temperatures) this is done for Haparanda
using a solid line. Note that doing this would make no sense for scatter
plots like the one in Figure \@ref(f-corruption1), because all the points
...
\@ref(f-temperatures) this is done by plotting the temperature
...
sense discussed in Section \@ref(s_means-dependent). There we considered
...
\@ref(f-temperatures) to *describe* such data.

### Linear associations {#ss_regression_descr_assoc}

Consider again statistically independent observations of $(X_{i},
Y_{i})$, such as those displayed in Figure \@ref(f-corruption1). Recall
...
\@ref(s_regression-simple).

![ The same scatterplot of Control of corruption vs. GDP per capita as
in Figure \@ref(f-corruption1), augmented by the best-fitting (least
squares) straight line (solid line) and reference lines for two example
values of Control of corruption (dotted lines).<span
data-label="f_corruption2"></span>](corruption2){width="13.5cm"}

Figure \@ref(f-corruption2) shows the same scatterplot as Figure
\@ref(f-corruption1). Consider first one value of $X$ (Control of
...
vertical line drawn at $X=65$ in Figure \@ref(f-corruption2). This can be
repeated for any value of $X$; for example, Figure \@ref(f-corruption2)
...
example, Figure \@ref(f-corruption2) suggests that the conditional mean
...
information in Figure \@ref(f-corruption2), for example, our best guesses
...
possibility is illustrated by plot (a) of Figure \@ref(f-scatterplots)
...
![ Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl1 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl2 "fig:"){width="8cm"}\
![ Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl3 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl4 "fig:"){width="8cm"}\
![ Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl5 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="f_scatterplots"></span>](olspl6 "fig:"){width="8cm"}\

In plot (a) of Figure \@ref(f-scatterplots) all the points are exactly on
...
illustrated by plot (b) of Figure \@ref(f-scatterplots), which shows a
...
\@ref(s_regression-simple) below.

The **direction** of a linear association can be either **positive** or
**negative**. Plots (a) and (b) of Figure \@ref(f-scatterplots) show a
...
by the remaining two plots of Figure \@ref(f-scatterplots). These
...
![A scatterplot of Control of corruption vs. GDP per capita for 163
countries in the Global Civil Society data set. The solid line is the
best-fitting (least squares) straight line for the points.<span
data-label="f_corruption3"></span>](corruption3){width="13.5cm"}
...
Figure \@ref(f-corruption2), also includes a best-fitting straight line.
...
them, shown in Figure \@ref(f-corruption3), the points with at least 60
on the $X$-axis are the same as those in Figure \@ref(f-corruption2), and
...
\@ref(f-scatterplots). The linear relationship which was a good
...
description of them until Section \@ref(ss_regression_simple-est). For
now we can treat the line simply as a visual summary of the linear
association in a scatterplot.

### Measures of association: covariance and correlation {#ss_regression_descr_corr}
...
recall from equation (\@ref(sd)) that the sample standard deviations of
the two variables are calculated as
$$s_{x} = \sqrt{\frac{\sum(X_{i}-\bar{X})^{2}}{n-1}}
\text{and}
s_{y} = \sqrt{\frac{\sum (Y_{i}-\bar{Y})^{2}}{n-1}}
\label{sdyx}$$ where the subscripts $x$ and $y$ identify the two
...
\label{sxy}$$ This is a measure of linear association between $X$ and
...
\label{corr}$$ This is the (sample) **correlation** coefficient, or
...
(of the $\chi^{2}$ test, see page \pageref{p_pearson}), although both
...
\@ref(ss_descr1_2cat-gamma). While doing so, it is useful to refer to the
examples in Figure \@ref(f-scatterplots), where the correlations are also
shown.
...
-   **Extreme values**: The largest possible correlation is $+1$
    ...
    Figure \@ref(f-scatterplots). In plot (e), there is curvature but
...
Returning to real data, Table \@ref(t_civilsoc-r) shows the correlation
...
\pageref{p_civilsoc}. This is an example of a **correlation matrix**,
...
definition and thus of no real interest.
  ...
  Society data set. See page \pageref{p_civilsoc} for more information
  on the variables.<span data-label="t_civilsoc_r"></span>
...
\@ref(f-corruption3). The correlation is high because this increase in
...
restricted set of countries shown in Figure \@ref(f-corruption2) is 0.82.
...
## Simple linear regression models {#s_regression_simple}

### Introduction {#ss_regression_simple_intro}
...
in subsequent sections. Section \@ref(s_regression-multiple) introduces
...
Section \@ref(s_regression-dummies). Finally, Section
\@ref(s_regression-rest) gives a brief review of some further aspects of
...
\@ref(f-imr1), together with the best-fitting straight line. Later we
...
categories (c.f. page \pageref{p_civilsoc}). For further reference,
Table \@ref(t-imrvars) shows various summary statistics for these
...
reason the correlations in Table \@ref(t-imrvars) differ slightly from
those in Table \@ref(t_civilsoc-r), where each correlation was calculated
for all the countries with non-missing values of that pair of variables.

![A scatterplot of net primary school enrolment ratio vs. Infant
...
line for the points.<span
data-label="f_imr1"></span>](imr1){width="13.5cm"}
  ...
  *Means for countries in different income categories*
  ...
  : Summary statistics for Infant Mortality Rate (IMR) and explanatory
  variables for it considered in the examples of Sections
  \@ref(s_regression_simple) and \@ref(s_regression-multiple) ($n=111$).
  See page \pageref{p_civilsoc} for further information on the
  variables.<span data-label="t_imrvars"></span>

### Definition of the model {#ss_regression_simple_def}
...
\@ref(s_means-inference). There the data consisted of observations
$(Y_{i},
...
combined as $$Y_{i}=\alpha+\beta X_{i} +\epsilon_{i}
\label{slinmodel}$$ where each $\epsilon_{i}$ is normally distributed
...
-   **Model**, because this is a statistical model in the sense
    discussed on page \pageref{p_model}. In other words, the model is
...
Section \@ref(s_regression-rest).

### Interpretation of the model parameters {#ss_regression_simple_int}
...
\@ref(f_spss-linreg). Note that although these values are first used here
...
![Illustration of the interpretation of the regression coefficients of a
simple linear regression model.<span
data-label="f_linmod_params"></span>](lmparams){width="12.5cm"}
...
\@ref(f_linmod-params). It is the only parameter in the model which
...
\@ref(f_linmod-params). If $\beta$ is negative, the regression line
...
In the real example shown in Figure \@ref(f_spss-linreg), $X$ is School
...
enrolment. This is the line shown in Figure \@ref(f-imr1).
...
\@ref(t-imrvars)). In such cases the results can easily be reexpressed by
...
Figure \@ref(f_linmod-params) or, say, school enrolment of 85 in Figure
\@ref(f-imr1). The model specifies a distribution for $Y$ given any such
...
\@ref(f-norm1)) on the vertical line from $X$, centered on the regression
...
### Estimation of the parameters {#ss_regression_simple_est}
...
These quantities are illustrated in Figure \@ref(f-residuals). This shows
a fitted regression line, which is in fact the one for IMR given School
enrolment also shown in Figure \@ref(f-imr1). Also shown are two points
$(X_{i}, Y_{i})$. These are also from Figure \@ref(f-imr1); the rest have
...
$\hat{\beta}=-0.179$ in Figure \@ref(f_spss-linreg), the fitted value for
...
![Illustration of the quantities involved in the definitions of least
squares estimates and the coefficient of determination $R^{2}$. See the
text for explanation.<span
data-label="f_residuals"></span>](lmresids){width="13.5cm"}
...
The second point shown in Figure \@ref(f-residuals) corresponds to the
...
parameter estimates in Figure \@ref(f_spss-linreg) were obtained. In
...
Figure \@ref(f-imr1). Each possible choice of $\hat{\alpha}$ and
...
{\sum (X_{i}-\bar{X})^{2}}
=\frac{s_{xy}}{s_{x}^{2}}
\label{ols_b}$$ and $$\hat{\alpha}=\bar{Y}-\hat{\beta}\bar{X}
\label{ols_a}$$ where $\bar{Y}$, $\bar{X}$, $s_{x}$ and $s_{xy}$ are the
...
residual standard deviation $\sigma$, is $$\hat{\sigma}=
\sqrt{
...
\label{sigma_linreg}$$ where $k$ is here set equal to 1. This bears an
obvious resemblance to the formula for the basic sample standard
deviation, shown for $Y_{i}$ in (\@ref(sdyx)). One difference to that
formula is that the denominator of (\@ref(sigma-linreg)) is shown as
...
$df=111-2=109$, as shown in Figure \@ref(f_spss-linreg).
...
-   The estimated slope $\hat{\beta}$ from (\@ref(ols-b)) is related to
    the sample correlation $r$ from (\@ref(corr)) by
...
-   Suppose we simplify the simple linear regression model
    (\@ref(slinmodel)) further by setting $\beta=0$, thus removing
    ...
    model considered in Section \@ref(s_means-1sample). Using the methods
    ...
    obtained by setting $\hat{\beta}=0$ in (\@ref(ols-a)). Since there is
    no $\hat{\beta}$ in this case, $\hat{Y}_{i}=\bar{Y}$ for all
    observations, $k=0$ and $df=n-(k+1)=n-1$. Substituting these into
    (\@ref(sigma-linreg)) shows that $\hat{\sigma}$ is then equal to the
...
observed data is then $Y_{i}-\bar{Y}$ (c.f. Figure \@ref(f-residuals) for
...
(\hat{Y}_{i}-\bar{Y})^{2}.
\label{ss_decomp}$$ Here $SSM=\sum (\hat{Y}_{i}-\bar{Y})^{2}=TSS-SSE$ is
...
this language, (\@ref(ss-decomp)) can be stated as
...
\sum (Y_{i}-\bar{Y})^{2}}.
\label{R2}$$ This is the *proportion* of the total variation of $Y$ in
...
association of the kind discussed in Section \@ref(ss_descr1_2cat-gamma),
with $E_{1}=TSS$ and $E_{2}=SSE$ in the notation of equation
(\@ref(PRE)). It is also related to the correlation coefficient. In
...
correlation $R$ next to it as “R”. Figure \@ref(f_spss-linreg) shows
...
-   Which values of $R^{2}$ are large or small or “good” is not a
    ...
    multiple linear model for IMR in Section \@ref(s_regression-dummies))
...
### Statistical inference for the regression coefficients {#ss_regression_simple_inf}
...
discussed here is that its value is zero, i.e. $$H_{0}:\; \beta=0.
\label{H0betasimple}$$ Recall that when $\beta$ is 0, there is no linear
...
\@ref(f-scatterplots) for an illustration of such a line in a sample).
The hypothesis (\@ref(H0betasimple)) can thus be expressed in words as
$$H_{0}:\; \text{*There is no
linear association
between* X \text{*and* }Y\text{ *in the population*}}.
\label{H0betasimple2}$$ Tests of this are usually carried out against a
...
\label{sebeta}$$ where $\hat{\sigma}$ is the estimated residual standard
deviation given by (\@ref(sigma-linreg)), and $s_{x}$ is the sample
...
$\beta$. The last expression in (\@ref(sebeta)) shows that the sample
...
table. Figure \@ref(f_spss-linreg) shows that
...
again of the general form (\@ref(ttest-gen)), i.e. a point estimate
divided by its standard error. Here this gives
$$t=\frac{\hat{\beta}}{\hat{\text{se}}(\hat{\beta})}.
\label{tbeta}$$ The logic of this is the same as in previous
applications of the same idea. Since the null hypothesis
(\@ref(H0betasimple)) claims that the population $\beta$ is zero, values
...
In the infant mortality example shown in Figure \@ref(f_spss-linreg), the
...
\label{cibeta}$$ where the multiplier $t_{\alpha/2}^{(n-2)}$ is obtained
from the $t_{n-2}$ distribution as in previous applications of $t$-based
confidence intervals (c.f. the description in Section
\@ref(ss_means_inference-variants)). For a 95% confidence interval
...
\newpage
## Interlude: Association and causality {#s_regression_causality}
...
as in the graph in Figure \@ref(f-xyzspurious). If we here consider only
...
![\label{fig:xyzspurious} A graphical representation of a
...
Chapters \@ref(c_probs} and \ref{c-means), Examples 5.3, 5.4 and 7.3 are
...
## Multiple linear regression models {#s_regression_multiple}

### Introduction {#ss_regression_multiple_intro}
...
is explained in Section \@ref(ss_regression_multiple-def) below. The
...
in Section \@ref(ss_regression_multiple-unchanged) cover much of the
multiple linear model simply by referring back to the descriptions in
Section \@ref(s_regression-simple). One aspect of the model is, however,
...
\@ref(ss_regression_multiple-beta). The crucial part of this
...
as measured by the Gini index (see p. \pageref{p_civilsoc}). Results for
this model are shown in Table \@ref(t_imr-m2), to which we will refer
...
\@ref(f_spss-linreg) is normally not appropriate in formal research
reports.
  ...
  : Response variable: Infant Mortality Rate (\%). Results for a linear regression model for Infant mortality rate
  given three explanatory variables in the Global Civil Society
  data. $\hat{\sigma}=2.23$; $R^{2}=0.692$; $n=111$; $df=107$<span data-label="t_imr_m2"></span>

### Definition of the model {#ss_regression_multiple_def}
...
\label{mu_multiple}$$ where the coefficients
$\beta_{1}, \beta_{2}, \dots, \beta_{k}$ of individual explanatory
variables are now also identified with subscripts. As in
(\@ref(slinmodel)) for the simple linear model, the multiple model can
also be expressed in the concise form $$Y_{i} = \alpha
+\beta_{1}X_{1i}+\beta_{2}X_{2i}+\dots+\beta_{k}X_{ki}+\epsilon_{i}
\label{mlinmodel}$$ where the error term (population residual)
$\epsilon_{i}$ is normally distributed with mean 0 and variance
$\sigma^{2}$.

The expected value of $Y$ as defined in (\@ref(mu-multiple)) is a linear
...
\@ref(ss_regression_multiple-beta) below.

### Unchanged elements from simple linear models {#ss_regression_multiple_unchanged}
...
-   The **constant term** (intercept) $\alpha$ is interpreted as the
    ...
    all to 0 in (\@ref(mu-multiple)). As before, $\alpha$ is rarely of
    any substantive interest. In the example in Table \@ref(t_imr-m2),
...
-   The **residual standard deviation** $\sigma$ is the standard
    ...
    variables. In Table \@ref(t_imr-m2), the estimate of $\sigma$
    is 2.23.
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    \hat{\beta}_{k}X_{ki}.
    \label{Yhat}$$ The estimated regression coefficients are again
    ...
    such as SPSS. In Table \@ref(t_imr-m2), the least squares estimates
    ...
    Table \@ref(t_imr-m2). The arrangement of other parts of SPSS output
    for multiple linear regression is essentially unchanged from the
    format shown in Figure \@ref(f_spss-linreg).

-   Predicted values for $Y$ can be calculated from (\@ref(Yhat)) for any
    ...
    implications of a fitted model. For example, Table \@ref(t-imrvars)
    shows that the sample averages of the explanatory variables in Table
    \@ref(t_imr_m2) are approximately 86 for School enrolment ($X-{1}$),
...
-   The estimated residual standard error $\hat{\sigma}$ is again
    calculated from (\@ref(sigma-linreg)), using the appropriate value of
...
-   The explanation of the coefficient of determination $R^{2}$ is
    entirely unchanged from the one given on page \pageref{p_R2}. It is
    still calculated with the formula (\@ref(R2)), and its interpretation
    ...
    \@ref(f_spss-linreg), adding the two new explanatory variables has
    increased $R^{2}$ by 12.5 percentage points, which seems like a
    reasonably large increase.

### Interpretation and inference for the regression coefficients {#ss_regression_multiple_beta}

#### Interpretation {-}

The concept of statistical control was outlined in Section
\@ref(s_regression-causality) above. In essence, its idea is to examine
...
\label{mu1}$$ for any values of $X_{1}$, $X_{2}$ and $X_{3}$. Suppose
...
\label{mu2}$$ Subtracting (\@ref(mu1}) from (\ref{mu2)) leaves us with
...
regression in Section \@ref(ss_regression_simple-int).
...
In the IMR example, the estimated coefficients in Table \@ref(t_imr-m2)
...
\@ref(t_imr-m2). A $t$-test statistic for the null hypothesis discussed
below is given by
$$t=\frac{\hat{\beta}_{j}}{\hat{\text{se}}(\hat{\beta}_{j})}.
\label{tbeta_m}$$ This is identical in form to statistic (\@ref(tbeta))
...
model, (\@ref(tbeta-m)) is a test statistic for the null hypothesis
$$H_{0}:\; \beta_{j}=0, \text{other regression coefficients
are unrestricted}
\label{H0beta_m}$$ against the alternative hypothesis
...
included in the model. In other words, (\@ref(tbeta-m)) tests
$$\begin{aligned}
...
The sampling distribution of (\@ref(tbeta-m)) when the null hypothesis
(\@ref(H0beta-m)) holds is a $t$ distribution with $n-(k+1)$ degrees of
...
regression models, such as those in Table \@ref(t_imr-m2). First,
(\@ref(H0beta-m)) implies that if the null hypothesis is not rejected,
...
(\@ref(H0beta-m)), one for each explanatory variable. This raises the
...
\@ref(s_regression-rest).

In the example shown in Table \@ref(t_imr-m2), the $P$-values are small
...
\label{cibeta_m}$$ This is identical in form and interpretation to the
interval (\@ref(cibeta)) for simple regression (except that the degrees
...
$t_{0.025}^{(107)}=1.98$) are shown in Table \@ref(t_imr-m2).

## Including categorical explanatory variables {#s_regression_dummies}

### Dummy variables {#ss_regression_dummies_def}
...
Summary statistics in Table \@ref(t-imrvars) show that income group is
...
\@ref(ss_regression_dummies-example) then describes a different example
...
results for this in our example are shown in Table \@ref(t_imr-m3). This
...
coefficients of the dummy variables.
  ...
  in Table \@ref(t_imr-m2) and Income level in three groups. $\hat{\sigma}=2.01$; $R^{2}=0.753$; $n=111$; $df=105$.<span
  data-label="t_imr_m3"></span>
...
\@ref(t_imr-m3), and the coefficients of the dummy variables for low and
...
\@ref(t_imr-m3) shows that the coefficients of both the middle income and
...
\@ref(t_imr-m3) are intervals for this difference.
...
levels. Comparing the results in Tables \@ref(t_imr-m2) and
\@ref(t_imr-m3), it can be seen that the effect of School enrolment
...
Table \@ref(t_imr-m3). This variable is strongly associated with income
(as seen in Table \@ref(t-imrvars)), with the more corrupt countries
...
Section \@ref(s_means-inference). Related to the notation of that
section, the coefficients from the model are $\hat{\alpha}=\bar{Y}_{1}$,
$\hat{\beta}=\bar{Y}_{2}-\bar{Y}_{1}$, and $\hat{\sigma}$ from
(\@ref(sigma-linreg)) is equal to (\@ref(sehatjoint)). Similarly, the
...
### A second example {#ss_regression_dummies_example}
...
Table \@ref(t_fastfood-descr).
  ...
  : Summary statistics for the variables considered in the minimum wage
  example of Section \@ref(ss_regression_dummies-example). Mean employment change: Among unaffected restaurants: $-2.93$; Among affected restaurants: $+0.68$.<span
  data-label="t_fastfood_descr"></span>
  ...
  : Response variable: Change in Full-time equivalent employment. Two fitted models for Employment change given exposure to minimum
  wage increase and control variables. See the text for further
  details.<span data-label="t_fastfood_models"></span>

Table \@ref(t_fastfood-models) shows results for two linear regression
...
in Table \@ref(t_fastfood-descr) show (although without controlling for
...
\@ref(t_fastfood-models) describe estimated differences between
...
from the estimates in Table \@ref(t_fastfood-models). This is not
entirely surprising, as the control variables are weakly associated with
the variable of interest: as seen in Table \@ref(t_fastfood-descr), the
...
\@ref(t_fastfood-models). The results are qualitatively the same as for
...
restaurants, which is 0.115 (c.f. Table \@ref(t_fastfood-descr)). The
...
## Other issues in linear regression modelling {#s_regression_rest}
...
Figure \@ref(f_spss-linreg). Most parts of it have been explained above,
...
-   The last two columns of the “**ANOVA**” (Analysis of Variance) table
    ...
    example, this could be used in the example of Table \@ref(t_imr-m3)
