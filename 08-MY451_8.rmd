...
data-label="fig:f-corruption1"></span>](corruption1){width="14cm"}
...
data-label="fig:f-temperatures"></span>](temperplot "fig:"){width="13cm"}\
...
data-label="fig:f-corruption2"></span>](corruption2){width="13.5cm"}
...
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl1 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl2 "fig:"){width="8cm"}\
![ Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl3 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl4 "fig:"){width="8cm"}\
![ Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl5 "fig:"){width="8cm"} ![
Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$. <span
data-label="fig:f-scatterplots"></span>](olspl6 "fig:"){width="8cm"}\
...
data-label="fig:f-corruption3"></span>](corruption3){width="13.5cm"}
...
Returning to real data, Table \@ref(t-civilsoc-r) shows the correlation
...
definition and thus of no real interest.
  ...
  on the variables.<span data-label="tab:t-civilsoc-r"></span>
...
those in Table \@ref(t-civilsoc-r), where each correlation was calculated
...
data-label="fig:f-imr1"></span>](imr1){width="13.5cm"}
  ...
  *Means for countries in different income categories*
  ...
  variables.<span data-label="tab:t-imrvars"></span>
...
\@ref(f-spss-linreg). Note that although these values are first used here
...
data-label="fig:f-linmod-params"></span>](lmparams){width="12.5cm"}
...
\@ref(f-linmod-params). It is the only parameter in the model which
...
\@ref(f-linmod-params). If $\beta$ is negative, the regression line
...
In the real example shown in Figure \@ref(f-spss-linreg), $X$ is School
...
Figure \@ref(f-linmod-params) or, say, school enrolment of 85 in Figure
...
$\hat{\beta}=-0.179$ in Figure \@ref(f-spss-linreg), the fitted value for
...
data-label="fig:f-residuals"></span>](lmresids){width="13.5cm"}
...
parameter estimates in Figure \@ref(f-spss-linreg) were obtained. In
...
$df=111-2=109$, as shown in Figure \@ref(f-spss-linreg).
...
correlation $R$ next to it as “R”. Figure \@ref(f-spss-linreg) shows
...
table. Figure \@ref(f-spss-linreg) shows that
...
In the infant mortality example shown in Figure \@ref(f-spss-linreg), the
...
this model are shown in Table \@ref(t-imr-m2), to which we will refer
...
\@ref(f-spss-linreg) is normally not appropriate in formal research
reports.
  ...
  data. $\hat{\sigma}=2.23$; $R^{2}=0.692$; $n=111$; $df=107$<span data-label="tab:t-imr-m2"></span>
...
-   The **constant term** (intercept) $\alpha$ is interpreted as the
    ...
    any substantive interest. In the example in Table \@ref(t-imr-m2),
...
-   The **residual standard deviation** $\sigma$ is the standard
    ...
    variables. In Table \@ref(t-imr-m2), the estimate of $\sigma$
    is 2.23.
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    such as SPSS. In Table \@ref(t-imr-m2), the least squares estimates
    ...
    Table \@ref(t-imr-m2). The arrangement of other parts of SPSS output
    for multiple linear regression is essentially unchanged from the
    format shown in Figure \@ref(f-spss-linreg).
...
-   Predicted values for $Y$ can be calculated from (\@ref(Yhat)) for any
    ...
    \@ref(t-imr-m2) are approximately 86 for School enrolment ($X-{1}$),
...
-   The explanation of the coefficient of determination $R^{2}$ is
    ...
    \@ref(f-spss-linreg), adding the two new explanatory variables has
...
In the IMR example, the estimated coefficients in Table \@ref(t-imr-m2)
...
\@ref(t-imr-m2). A $t$-test statistic for the null hypothesis discussed
below is given by
...
regression models, such as those in Table \@ref(t-imr-m2). First,
...
In the example shown in Table \@ref(t-imr-m2), the $P$-values are small
...
$t_{0.025}^{(107)}=1.98$) are shown in Table \@ref(t-imr-m2).
...
results for this in our example are shown in Table \@ref(t-imr-m3). This
...
coefficients of the dummy variables.
  ...
  in Table \@ref(t-imr-m2) and Income level in three groups. $\hat{\sigma}=2.01$; $R^{2}=0.753$; $n=111$; $df=105$.<span
  data-label="tab:t-imr-m3"></span>
...
\@ref(t-imr-m3), and the coefficients of the dummy variables for low and
...
\@ref(t-imr-m3) shows that the coefficients of both the middle income and
...
\@ref(t-imr-m3) are intervals for this difference.
...
levels. Comparing the results in Tables \@ref(t-imr-m2) and
\@ref(t-imr-m3), it can be seen that the effect of School enrolment
...
Table \@ref(t-imr-m3). This variable is strongly associated with income
...
Table \@ref(t-fastfood-descr).
  ...
  example of Section \@ref(ss-regression-dummies-example). Mean employment change: Among unaffected restaurants: $-2.93$; Among affected restaurants: $+0.68$.<span
  data-label="tab:t-fastfood-descr"></span>
  ...
  details.<span data-label="tab:t-fastfood-models"></span>

Table \@ref(t-fastfood-models) shows results for two linear regression
...
in Table \@ref(t-fastfood-descr) show (although without controlling for
...
\@ref(t-fastfood-models) describe estimated differences between
...
from the estimates in Table \@ref(t-fastfood-models). This is not
entirely surprising, as the control variables are weakly associated with
the variable of interest: as seen in Table \@ref(t-fastfood-descr), the
...
\@ref(t-fastfood-models). The results are qualitatively the same as for
...
restaurants, which is 0.115 (c.f. Table \@ref(t-fastfood-descr)). The
...
Figure \@ref(f-spss-linreg). Most parts of it have been explained above,
...
-   The last two columns of the “**ANOVA**” (Analysis of Variance) table
    ...
    example, this could be used in the example of Table \@ref(t-imr-m3)
