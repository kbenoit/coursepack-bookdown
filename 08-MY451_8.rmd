...
\label{eq:sdyx}$$ where the subscripts $x$ and $y$ identify the two
...
\label{eq:sxy}$$ This is a measure of linear association between $X$ and
...
\label{eq:corr}$$ This is the (sample) **correlation** coefficient, or
...
(of the $\chi^{2}$ test, see footnote 1 in Chapter \@ref(c-tables)), although both
...
\label{eq:slinmodel}$$ where each $\epsilon_{i}$ is normally distributed
...
{\sum (X_{i}-\bar{X})^{2}}
=\frac{s_{xy}}{s_{x}^{2}}
\label{eq:ols_b}$$ and $$\hat{\alpha}=\bar{Y}-\hat{\beta}\bar{X}
\label{eq:ols_a}$$ where $\bar{Y}$, $\bar{X}$, $s_{x}$ and $s_{xy}$ are the
...
\label{eq:sigma_linreg}$$ where $k$ is here set equal to 1. This bears an
...
(\hat{Y}_{i}-\bar{Y})^{2}.
\label{eq:ss_decomp}$$ Here $SSM=\sum (\hat{Y}_{i}-\bar{Y})^{2}=TSS-SSE$ is
...
\sum (Y_{i}-\bar{Y})^{2}}.
\label{eq:R2}$$ This is the *proportion* of the total variation of $Y$ in
...
\label{eq:H0betasimple}$$ Recall that when $\beta$ is 0, there is no linear
...
\label{eq:H0betasimple2}$$ Tests of this are usually carried out against a
...
\label{eq:sebeta}$$ where $\hat{\sigma}$ is the estimated residual standard
...
\label{eq:tbeta}$$ The logic of this is the same as in previous
...
\label{eq:cibeta}$$ where the multiplier $t_{\alpha/2}^{(n-2)}$ is obtained
...
\label{eq:mu_multiple}$$ where the coefficients
...
\label{eq:mlinmodel}$$ where the error term (population residual)
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    \hat{\beta}_{k}X_{ki}.
    \label{eq:Yhat}$$ The estimated regression coefficients are again
...
\label{eq:mu1}$$ for any values of $X_{1}$, $X_{2}$ and $X_{3}$. Suppose
...
\label{eq:mu2}$$ Subtracting (\@ref(mu1}) from (\ref{mu2)) leaves us with
...
\label{eq:tbeta_m}$$ This is identical in form to statistic (\@ref(tbeta))
...
are unrestricted}
\label{eq:H0beta_m}$$ against the alternative hypothesis
...
\label{eq:cibeta_m}$$ This is identical in form and interpretation to the
