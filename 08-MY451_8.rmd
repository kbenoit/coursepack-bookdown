...
recall from equation (\@ref(eq:sd)) that the sample standard deviations of
...
(\@ref(eq:PRE)). It is also related to the correlation coefficient. In
...
again of the general form (\@ref(eq:ttest-gen)), i.e. a point estimate
...
(\@ref(eq:sigma-linreg)) is equal to (\@ref(eq:sehatjoint)). Similarly, the
standard error (\@ref(eq:sebeta)) is the same as
$\hat{\sigma}_{\hat{\Delta}}$ in (\@ref(eq:seD2)), and the test statistic
(\@ref(eq:tbeta)) and confidence interval (\@ref(eq:cibeta)) are identical with
(\@ref(eq:ztestmuDb)) and the $t$ distribution version of (\@ref(eq:ciDmu2))
respectively.
