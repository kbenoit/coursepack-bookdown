...
\ref{s_regression_descr} first discusses graphical and numerical
...
\ref{s_regression_simple} then describes methods of statistical
...
analysis briefly in Section \ref{s_regression_causality}. Multiple
linear models are then introduced in Section
\ref{s_regression_multiple}. The models can also include categorical
...
\ref{ss_regression_descr_corr}. Statistical *inference* for the
...
in Figure \ref{f_corruption1}, which shows a scatterplot of Control of
...
example (another one is Figure \ref{f_houseprices} on page
...
$Y$-axis. In Figure \ref{f_temperatures} this is done for Haparanda
using a solid line. Note that doing this would make no sense for scatter
plots like the one in Figure \ref{f_corruption1}, because all the points
...
\ref{f_temperatures} this is done by plotting the temperature
...
\ref{f_temperatures} to *describe* such data.
...
In the two-sample examples of Chapter \ref{c_means} this could be
...
\ref{s_regression_simple}.

![ The same scatterplot of Control of corruption vs. GDP per capita as
in Figure \ref{f_corruption1}, augmented by the best-fitting (least
...
vertical line drawn at $X=65$ in Figure \ref{f_corruption2}. This can be
repeated for any value of $X$; for example, Figure \ref{f_corruption2}
also includes a vertical reference line at $X=95$, for examining the
conditional distribution of $Y$ given $X=95$[^4].

As in Chapter \ref{c_means}, associations between variables will here be
...
information in Figure \ref{f_corruption2}, for example, our best guesses
...
possibility is illustrated by plot (a) of Figure \ref{f_scatterplots}
...
illustrated by plot (b) of Figure \ref{f_scatterplots}, which shows a
...
\ref{s_regression_simple} below.

The **direction** of a linear association can be either **positive** or
**negative**. Plots (a) and (b) of Figure \ref{f_scatterplots} show a
...
\ref{f_scatterplots}. The linear relationship which was a good
...
description of them until Section \ref{ss_regression_simple_est}. For
...
\ref{ss_descr1_2cat_gamma}. While doing so, it is useful to refer to the
examples in Figure \ref{f_scatterplots}, where the correlations are also
shown.
...
-   **Extreme values**: The largest possible correlation is $+1$
    ...
    Figure \ref{f_scatterplots}. In plot (e), there is curvature but
...
\ref{f_corruption3}. The correlation is high because this increase in
...
restricted set of countries shown in Figure \ref{f_corruption2} is 0.82.
...
in subsequent sections. Section \ref{s_regression_multiple} introduces
...
Section \ref{s_regression_dummies}. Finally, Section
\ref{s_regression_rest} gives a brief review of some further aspects of
...
\ref{f_imr1}, together with the best-fitting straight line. Later we
...
Table \ref{t_imrvars} shows various summary statistics for these
...
data-label="f_imr1"></span>](imr1){width="13.5cm"}
  ...
  *Means for countries in different income categories*
  ...
  \ref{s_regression_simple} and \ref{s_regression_multiple} ($n=111$).
...
\ref{s_means_inference}. There the data consisted of observations
$(Y_{i},
...
\ref{f_spss_linreg}. Note that although these values are first used here
...
enrolment. This is the line shown in Figure \ref{f_imr1}.
...
Figure \ref{f_linmod_params} or, say, school enrolment of 85 in Figure
\ref{f_imr1}. The model specifies a distribution for $Y$ given any such
...
enrolment also shown in Figure \ref{f_imr1}. Also shown are two points
$(X_{i}, Y_{i})$. These are also from Figure \ref{f_imr1}; the rest have
...
$\hat{\beta}=-0.179$ in Figure \ref{f_spss_linreg}, the fitted value for
...
formula is that the denominator of (\ref{sigma_linreg}) is shown as
...
$df=111-2=109$, as shown in Figure \ref{f_spss_linreg}.
...
-   The estimated slope $\hat{\beta}$ from (\ref{ols_b}) is related to
...
-   Suppose we simplify the simple linear regression model
    ...
    model considered in Section \ref{s_means_1sample}. Using the methods
    ...
    obtained by setting $\hat{\beta}=0$ in (\ref{ols_a}). Since there is
    no $\hat{\beta}$ in this case, $\hat{Y}_{i}=\bar{Y}$ for all
    observations, $k=0$ and $df=n-(k+1)=n-1$. Substituting these into
    (\ref{sigma_linreg}) shows that $\hat{\sigma}$ is then equal to the
    usual sample standard deviation $s_{y}$ of $Y_{i}$.

#### Coefficient of determination ($R^{2}$) {#p_R2}
...
observed data is then $Y_{i}-\bar{Y}$ (c.f. Figure \ref{f_residuals} for
...
this language, (\ref{ss_decomp}) can be stated as
...
association of the kind discussed in Section \ref{ss_descr1_2cat_gamma},
...
correlation $R$ next to it as “R”. Figure \ref{f_spss_linreg} shows
...
-   Which values of $R^{2}$ are large or small or “good” is not a
    ...
    multiple linear model for IMR in Section \ref{s_regression_dummies})
...
deviation given by (\ref{sigma_linreg}), and $s_{x}$ is the sample
...
table. Figure \ref{f_spss_linreg} shows that
...
tests of one and two means discussed in Chapter \ref{c_means}, it would
...
\ref{ss_means_inference_variants}). For a 95% confidence interval
...
as in the graph in Figure \ref{f_xyzspurious}. If we here consider only
...
Chapters \ref{c_probs} and \ref{c_means}, Examples 5.3, 5.4 and 7.3 are
...
Section \ref{s_regression_simple}. One aspect of the model is, however,
...
this model are shown in Table \ref{t_imr_m2}, to which we will refer
...
\ref{f_spss_linreg} is normally not appropriate in formal research
reports.
...
-   The **constant term** (intercept) $\alpha$ is interpreted as the
    ...
    all to 0 in (\ref{mu_multiple}). As before, $\alpha$ is rarely of
    any substantive interest. In the example in Table \ref{t_imr_m2},
...
-   The **residual standard deviation** $\sigma$ is the standard
    ...
    variables. In Table \ref{t_imr_m2}, the estimate of $\sigma$
    is 2.23.
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    such as SPSS. In Table \ref{t_imr_m2}, the least squares estimates
    ...
    Table \ref{t_imr_m2}. The arrangement of other parts of SPSS output
...
-   Predicted values for $Y$ can be calculated from (\ref{Yhat}) for any
    ...
    implications of a fitted model. For example, Table \ref{t_imrvars}
    shows that the sample averages of the explanatory variables in Table
    \ref{t_imr_m2} are approximately 86 for School enrolment ($X_{1}$),
...
-   The estimated residual standard error $\hat{\sigma}$ is again
    calculated from (\ref{sigma_linreg}), using the appropriate value of
...
-   The explanation of the coefficient of determination $R^{2}$ is
    ...
    \ref{f_spss_linreg}, adding the two new explanatory variables has
...
\ref{s_regression_causality} above. In essence, its idea is to examine
...
regression in Section \ref{ss_regression_simple_int}.
...
In the IMR example, the estimated coefficients in Table \ref{t_imr_m2}
...
\ref{t_imr_m2}. A $t$-test statistic for the null hypothesis discussed
below is given by
...
model, (\ref{tbeta_m}) is a test statistic for the null hypothesis
...
(\ref{H0beta_m}) holds is a $t$ distribution with $n-(k+1)$ degrees of
...
(\ref{H0beta_m}), one for each explanatory variable. This raises the
...
\ref{ss_regression_dummies_example} then describes a different example
...
coefficients of the dummy variables.
  ...
  in Table \ref{t_imr_m2} and Income level in three groups. $\hat{\sigma}=2.01$; $R^{2}=0.753$; $n=111$; $df=105$.<span
...
\ref{t_imr_m3} are intervals for this difference.
...
Section \ref{s_means_inference}. Related to the notation of that
...
Table \ref{t_fastfood_descr}.
  ...
  example of Section \ref{ss_regression_dummies_example}. Mean employment change: Among unaffected restaurants: $-2.93$; Among affected restaurants: $+0.68$.<span
...
from the estimates in Table \ref{t_fastfood_models}. This is not
entirely surprising, as the control variables are weakly associated with
the variable of interest: as seen in Table \ref{t_fastfood_descr}, the
...
-   The last two columns of the “**ANOVA**” (Analysis of Variance) table
    ...
    example, this could be used in the example of Table \ref{t_imr_m3}
...
[^12]: It would have been more consistent with related notation used in
    Chapter \ref{c_means} to denote it something like
