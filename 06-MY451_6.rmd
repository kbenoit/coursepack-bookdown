...
$$\mu=\frac{\sum Y_{i}}{N}
\label{eq:mu}$$ and the **population variance**
$$\sigma^{2} = \frac{\sum (Y_{i}-\mu)^{2}}{N}
\label{eq:sigma2}$$ or, instead of the variance, the **population standard
deviation** $$\sigma = \sqrt{\frac{\sum (Y_{i}-\mu)^{2}}{N}}.
\label{eq:sigma}$$ Here $\mu$ and $\sigma$ are the lower-case Greek letters
...
$$\begin{aligned}
\bar{Y}&=&\frac{\sum Y_{i}}{n}, \label{eq:Ybar_ch6} \\
s^{2} &=& \frac{\sum (Y_{i}-\bar{Y})^{2}}{n-1} \text{and}
\label{eq:s2_ch6} \\
s &=& \sqrt{\frac{\sum (Y_{i}-\bar{Y})^{2}}{n-1}}
\label{eq:s_ch6}\end{aligned}$$ where the sums are now over the $n$
...
\label{eq:Zscore}$$ where $Y$ can be any value of the variable of interest.
