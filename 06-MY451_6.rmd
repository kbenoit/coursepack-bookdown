...
-   As population distributions of continuous variables, for instance
    ...
    in Chapters \ref{c_tables} and \ref{c_probs}. Methods of inference
    for continuous variables will be introduced in Chapters
    \ref{c_means} and \ref{c_regression}.
...
-   As sampling distributions of sample statistics. These are typically
    ...
    Chapter \ref{c_probs} where the variable of interest $Y$ was binary
...
short physical examination by a nurse. Figure \ref{f_bp1} shows a
...
distribution by, say, a histogram like Figure \ref{f_bp1}. We can also
consider specific characteristics of the distribution, i.e.\its
*parameters* in the sense introduced in Section
\ref{s_probs_distribution}. For the distribution of a continuous
...
also for the probability parameter $\pi$ in Chapter \ref{c_probs}.
...
units. If we treat the data in Figure \ref{f_bp1} as a population,
...
of some sample statistics introduced in Chapter \ref{c_descr1},
...
sample distribution as discussed in Chapter \ref{c_descr1}, but also as
*point estimates* of the corresponding population parameters in the
sense defined in Section \ref{s_probs_pointest}. We may thus use the
...
corresponding parameters (\ref{mu})–(\ref{sigma}) in the population.
  ...
  \ref{s_contd_popdistrs}–\ref{s_contd_clt}.<span
...
polygon as in Figure \ref{f_bp1}, now supplemented by a smooth curve.
...
well described by a smooth curve such as the one in Figure \ref{f_bp2}.
...
![The frequency polygon of Figure \ref{f_bp1}, together with a normal
...
the distribution shown in Figure \ref{f_bp2} is entirely determined by
...
the *Binomial* distribution discussed in Chapter \ref{c_probs}, or the
...
\ref{f_bp2} is a **normal distribution**, also known as the *Gaussian*
distribution, after the great German mathematician Karl Friedrich Gauss
who was one of the first to derive it in 1809. Figure \ref{f_10dm} shows
...
distributions. For example, the curve in Figure \ref{f_bp2} is that of
...
curve best matches the frequency polygon in Figure \ref{f_bp2}.
...
\ref{f_3norms}, which shows the curves for three different normal
...
below). In Figure \ref{f_3norms}, the curves for $N(0, 1)$ and $N(0, 9)$
...
$P$-values for tests in Sections \ref{ss_tables_chi2test_Pval} and
...
Figure \ref{f_norm1} illustrates this further with some results which
...
Figure \ref{f_norm1}). Because the normal distribution is symmetric,
...
from it is shown in Table \ref{t_normtab} (note that it is not always
...
as in Figure \ref{f_normbp}.

The second quantity of interest was the probability of a blood pressure
at most 60, i.e. the area of the “Low” region in Figure \ref{f_normbp}.
...
\ref{ss_probs_test1sample_samplingd}, in the role of the *sampling
...
inference on proportions in Chapter \ref{c_probs}, and even for the
...
-   The variance of the sampling distribution of $\bar{Y}$ is
    ...
    **standard error of the mean**, and is often denoted by something
    ...
    is illustrated in Figure \ref{f_sampld2}. It shows the sampling
    ...
    \ref{ss_contd_probdistrs_normal} that approximately 95% of the
...
the sampling error, i.e.\the remaining uncertainty about population
...
later, in Section \ref{ss_means_tests3_power}.
...
these 100,000 values of $\bar{Y}$ is shown in Figure \ref{f_sampld}.
...
in Figure \ref{f_bp1}, for which the mean is $\mu=74.2$ and standard
...
Figure \ref{f_sampld}, on the other hand, is based on repeated samples
from the actual population distribution of blood pressure, which, while
quite close to a normal distribution as shown in Figure \ref{f_bp2}, is
...
\ref{pihat_as_ybar} on page \pageref{pihat_as_ybar}). The CLT then tells
us that
...
similar to Figure \ref{f_sampld} in that it shows sampling distributions
...
In Figure \ref{f_cltbin}, the observed range of values of $\hat{\pi}$
...
Chapter \ref{c_tables}. Results like these, and many others, explain the
