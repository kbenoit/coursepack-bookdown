# Continuous variables: Population and sampling distributions {#c_contd}

## Introduction {#s_contd_intro}
...
-   As population distributions of continuous variables, for instance
    ...
    in Chapters \@ref(c_tables) and \@ref(c-probs). Methods of inference
    for continuous variables will be introduced in Chapters
    \@ref(c_means) and \@ref(c-regression).
...
\label{p_bpressure_ex} To illustrate the concepts, we use data from the
Health Survey for England 2002 (HES).[^1] One part of the survey was a
short physical examination by a nurse. Figure \@ref(f-bp1) shows a
...
![Histogram of diastolic blood pressure, with the corresponding
frequency polygon, from Health Survey for England 2002 (respondents aged
25 or over, $n=4489$).<span
data-label="f_bp1"></span>](bloodp1){width="13.5cm"}

The respondents whose blood pressures are summarized in Figure
\@ref(f-bp1) are in reality a sample from a larger population in the
sense of Sections \@ref(s_samples_finpops) and \@ref(s_samples-samples).
...
small town, say). The values summarised in Figure \@ref(f-bp1) then form
...
## Population distributions of continuous variables {#s_contd_popdistrs}

### Population parameters and their point estimates {#ss_contd_popdistrs_params}

If we knew all of its values, we could summarise a finite population
distribution by, say, a histogram like Figure \@ref(f-bp1). We can also
consider specific characteristics of the distribution, i.e. its
*parameters* in the sense introduced in Section
\@ref(s_probs-distribution). For the distribution of a continuous
variable, the most important parameters are the **population mean**
$$\mu=\frac{\sum Y_{i}}{N}
\label{mu}$$ and the **population variance**
$$\sigma^{2} = \frac{\sum (Y_{i}-\mu)^{2}}{N}
\label{sigma2}$$ or, instead of the variance, the **population standard
deviation** $$\sigma = \sqrt{\frac{\sum (Y_{i}-\mu)^{2}}{N}}.
\label{sigma}$$ Here $\mu$ and $\sigma$ are the lower-case Greek letters
...
units. If we treat the data in Figure \@ref(f-bp1) as a population,
...
\@ref(s_samples-finpops)) but not to infinite ones of the kind discussed
in Section \@ref(s_samples-infpops). However, the definitions of $\mu$,
...
The population formulas (\@ref(mu})–(\ref{sigma)) clearly resemble those
...
\label{s_ch6}\end{aligned}$$ where the sums are now over the $n$
...
sense defined in Section \@ref(s_probs-pointest). We may thus use the
...
Figure \@ref(f_bp1). Table \@ref(t_bp-example) shows the summary
statistics (\@ref(Ybar_ch6))–(\@ref(s-ch6)) in this sample and the
corresponding parameters (\@ref(mu))–(\@ref(sigma)) in the population.
  ...
  : Summary statistics for diastolic blood pressure in the population
  and a sample from it in the example used for illustration in Sections
  \@ref(s_contd_popdistrs)–\@ref(s_contd-clt).<span
  data-label="t_bp_example"></span>

You may have noticed that the formulas of the sample variance
(\@ref(s2_ch6)) and sample standard deviation (\@ref(s-ch6)) involve the
...
which use $n$ instead,
i.e. $$\hat{\sigma}^{2}=\frac{\sum (Y_{i}-\bar{Y})^{2}}{n}
\label{s2b}$$ for $\sigma^{2}$ and
...
## Probability distributions of continuous variables {#s_contd_probdistrs}

### General comments {#ss_contd_probdistrs_general}

Thinking about population distributions of continuous distributions
using, say, histograms as in Figure \@ref(f-bp1) would present
...
small kinks and bumps of Figure \@ref(f-bp1). Such details would
...
Section \@ref(s_samples-popdistrs), and considered instances of it in
Chapters \@ref(c_tables} and \ref{c-probs). There, however, the term was
...
illustrated by Figure \@ref(f-bp2), which shows the same frequency
polygon as in Figure \@ref(f-bp1), now supplemented by a smooth curve.
...
well described by a smooth curve such as the one in Figure \@ref(f-bp2).

In Figure \@ref(f-bp2) the curve and the frequency polygon have
...
are clearly not exactly the same, \label{p_model2} nor do we expect that
...
![The frequency polygon of Figure \@ref(f-bp1), together with a normal
curve with the same mean and variance.<span
data-label="f_bp2"></span>](bloodp2){width="11.5cm"}
...
the distribution shown in Figure \@ref(f-bp2) is entirely determined by
...
\@ref(f-bp2), which is known as the normal distribution, is by definition
...
### The normal distribution as a population distribution {#ss_contd_probdistrs_normal}

The particular probability distribution that is included in Figure
\@ref(f-bp2) is a **normal distribution**, also known as the *Gaussian*
distribution, after the great German mathematician Karl Friedrich Gauss
who was one of the first to derive it in 1809. Figure \@ref(f-10dm) shows
...
![A portrait of Gauss and the normal curve on a former German 10-DM
banknote.<span data-label="f_10dm"></span>](mark){width="14cm"}
...
explained in Section \@ref(s_contd-clt). However, the normal distribution
...
\@ref(f-3norms), which shows the curves for three different normal
...
below). In Figure \@ref(f-3norms), the curves for $N(0, 1)$ and $N(0, 9)$
...
![Three normal distributions with different means and/or variances.<span
data-label="f_3norms"></span>](threenorms){width="11cm"}

The variance $\sigma^{2}$ determines how widely spread the curve is. In
Figure \@ref(f-3norms), the curves for $N(0, 1)$ and $N(5, 1)$ have the
...
In the histogram in Figure \@ref(f-bp1), the heights of the bars
...
$P$-values for tests in Sections \@ref(ss_tables_chi2test-Pval) and
\@ref(ss_probs_test1sample-samplingd).

![Illustration of probabilities for the normal distribution. The
...
0.95.<span data-label="f_norm1"></span>](norm1){width="12cm"}

Figure \@ref(f-norm1) illustrates this further with some results which
...
Figure \@ref(f-norm1)). Because the normal distribution is symmetric,
...
are then the areas of the regions shown in Figure \@ref(f-normbp).
...
defined as $$Z = \frac{Y-\mu}{\sigma}
\label{Zscore}$$ where $Y$ can be any value of the variable of interest.
...
![ Illustration of probabilities for a normal distribution in the blood
...
and 90 ("Mid”) and over 90 (“High”). <span
data-label="f_normbp"></span>](normbp){width="12cm"}
...
\footnotesize
  ...
  : Extract from the table of right-hand tail probabilities for normal
  ...
  labelled “$z$”. The full table is shown on
  p. \pageref{s_disttables_Z}.<span data-label="t_normtab"></span>
...
given on page \pageref{s_disttables_Z} in the Appendix, and an extract
from it is shown in Table \@ref(t-normtab) (note that it is not always
...
i.e. the area labelled “High” in Figure \@ref(f-normbp). We have seen
...
as in Figure \@ref(f-normbp).

The second quantity of interest was the probability of a blood pressure
at most 60, i.e. the area of the “Low” region in Figure \@ref(f-normbp).
...
\label{p_cdfnorm} These days we can use statistical computer programs to
...
for the sample in Table \@ref(t_bp-example) we had $\bar{Y}=72.6$ and
...
## The normal distribution as a sampling distribution {#s_contd_clt}

We have already encountered the normal distribution in Section
\@ref(ss_probs_test1sample-samplingd), in the role of the *sampling
...
Recall from Section \@ref(ss_tables_chi2test-sdist) that the sampling
...
-   The variance of the sampling distribution of $\bar{Y}$ is
    ...
    **standard error of the mean**, and is often denoted by something
    ...
    is illustrated in Figure \@ref(f-sampld2). It shows the sampling
    ...
    \@ref(ss_contd_probdistrs-normal) that approximately 95% of the
...
![Illustration of the sampling distribution of the sample mean for two
sample sizes. In both cases the population distribution is normal with
$\mu=74.2$ and $\sigma=11.3$. <span
data-label="f_sampld2"></span>](sampld2_bp){width="12cm"}
...
later, in Section \@ref(ss_means_tests3-power).

In Figure \@ref(f-sampld) we use a computer simulation rather than a
...
these 100,000 values of $\bar{Y}$ is shown in Figure \@ref(f-sampld).
...
![Example of the sampling distribution of the sample mean. The plot
...
in Figure \@ref(f-bp1), for which the mean is $\mu=74.2$ and standard
...
$\mu$ and standard deviation $\sigma/\sqrt{n}$. <span
data-label="f_sampld"></span>](sampld1_bp){width="12cm"}

The match between the curve and the histogram in Figure \@ref(f-sampld)
...
inference. Recall that the normal curve shown in Figure \@ref(f-sampld)
...
Figure \@ref(f-sampld), on the other hand, is based on repeated samples
from the actual population distribution of blood pressure, which, while
quite close to a normal distribution as shown in Figure \@ref(f-bp2), is
...
We have seen in Figure \@ref(f-sampld2) that in the blood pressure
...
\@ref(s_probs-distribution). If the probability parameter of this
distribution is $\pi$, its mean and variance are $\mu=\pi$ and
$\sigma^{2}=\pi(1-\pi)$, and the sample mean $\bar{Y}$ of observations
from the distribution is the sample proportion $\hat{\pi}$ (see equation
\@ref(pihat_as_ybar) on page \pageref{pihat_as-ybar}). The CLT then tells
us that
...
This powerful result is illustrated in Figure \@ref(f-cltbin). It is
similar to Figure \@ref(f-sampld) in that it shows sampling distributions
...
![Illustration of the Central Limit Theorem for the sample proportion of
...
sizes $n$ are 10, 30, 100 and 1000.<span
data-label="f_cltbin"></span>](sampld_p){width="140mm"}

The variability of the sampling distribution will again depend on $n$.
In Figure \@ref(f-cltbin), the observed range of values of $\hat{\pi}$
...
[^4]: The CLT does not hold in some rather weird cases which need not
    concern us here. Condition (\@ref(lindeberg)) is a mathematical
