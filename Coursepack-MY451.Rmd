...
-   **Other text books**: There are hundreds of introductory textbooks
    ...
    -   There are also many books which focus on the SPSS statistical
        ...
        books are mentioned at the beginning of Section \@ref(s-intro-SPSS).
...
Survey (ESS), which was carried out in 2010.[^1] The survey was fielded
...
it have been included in many public opinion surveys.[^2] Of key
...
![(\#fig:f-bars-attitude) Bar chart of the attitude variable in the survey data example. Agreement with statement: ``The government should take measures to reduce differences in income levels''.
European Social Survey, Round 5 (2010), UK respondents only.](bar_attitude){height="8cm"}
...
redistribution.[^3] As possible explanations for this pattern, both
...
U.S. brokerage house.[^4] One of the questions asked the respondents to
...
![(\#fig:f-boxplot-gdp) An annotated box plot of GDP per capita in the country data
($n=155$).](box_gdp){width="15cm"}
...
<http://www.ccsr.ac.uk/sars/>.](twohists "fig:"){width="14cm"}
...
Manitoba[^5]. As part of an initial description of these communities,
...
These are again in the expected order.[^6] For the country data, the
...
needed.[^7] Instead, we eliminate the signs of the deviations by using
...
`www.datavis.ca/gallery/index.php`, on the web page maintained by
...
-   **democracy** is a measure of institutionalised democracy by the
    Polity IV project[^8]. The values refer to each country’s
...
[^2]: For recent findings, see for example Svallfors, S. (ed.) (2012),
    *Contested Welfare States: Welfare Attitudes in Europe and Beyond*.
    Stanford University Press.

[^3]: See, for example, Svallfors (1997), Words of welfare and attitudes
...
[^4]: Lewellen, W. G., Lease, R. G., and Schlarbaum, G. G. (1977).
...
[^5]: This is a random sample of municipalities, obtained for this
    illustration from the 2001 census data provided by Statistics Canada
    at `www.statcan.gc.ca`.

[^6]: There is no need to worry about how the quartile values 9.25 and
    2.75 for class 3 were calculated. Different software packages may in
    fact do that slightly differently; these values are from SPSS.

[^7]: In mathematical terms, the difficulty is that the absolute value
    function has no derivative at zero.

[^8]: Monty G. Marshall and Keith Jaggers (2002). *Polity IV Dataset*.
    \[Computer file; version p4v2002\] College Park, MD: Center for
    International Development and Conflict Management, University of
    Maryland.

<!--chapter:end:02-MY451_2.rmd-->
...
test”, after Karl Pearson who first proposed it in 1900.[^1] We use this
...
![(\#fig:f-chisampld) Example of the sampling distribution of the $\chi^{2}$ test statistic
for independence. The plot shows a histogram of the values of the
statistic in 100,000 simulated samples of size $n=2344$ drawn from the
population distribution in the upper part of Table
\@ref(tab:t-sex-attitude-H0pop). Superimposed on the histogram is the curve
of the approximate sampling distribution, which is the $\chi^{2}$
distribution with 4 degrees of freedom.](chi2sims){width="8.5cm"}
...
relates to the structure of the table[^2]. We can, however, ignore this
...
![(\#fig:f-pvalchisq) Illustration of the $P$-value for a $\chi^{2}$ test statistic with 4
degrees of freedom and with values $\chi^{2}=2.84$ (area of the grey
region under the curve) and $\chi^{2}=18.86$.](chi2_pval){width="8cm"}
...
Such a table is also included in Section \@ref(s-disttables-chi2) of
...
[^1]: *Philosophical Magazine*, Series 5, **5**, 157–175.
...
[^2]: In short, they are the smallest number of cell frequencies such
    that they together with the row and column marginal totals are
    enough to determine all the remaining cell frequencies.

[^3]: There is an alternative framework, known as *Bayesian* statistics,
...
and 182 (25.9%) were still undecided[^1]. Here we will consider the
...
in the justice system in the U.S. state of Pennsylvania[^2], an
...
1954[^3]. In the randomized, double-blind placebo-control part of the
...
examined acquiescence bias through a randomized experiment[^4]. In a
...
here $\pi$ (the lower-case Greek letter “pi”[^5]). The value of $\pi$ is
...
\; $” or “hat” is often used to denote an estimate of a parameter
...
for the standard normal distribution. These values are also shown in Section \@ref(s-disttables-t) at the end of this course pack, on the
...
books.
  ...
  standard statistical tables (see the last row of the table in Section
  \@ref(s-disttables-t)). The lower part of the table includes the
  same numbers rearranged to show critical values for conventional
  significance levels for one- and two-sided tests.

  ------------------------------------ ------------------------------------------ ------ ------ -------
                                                              Significance levels               
  Alternative hypothesis                                                     0.10   0.05   0.01   0.001
...
children[^6]. This was acknowledged as a convincing demonstration that
...
the whole world (see
`www.polioeradication.org/Dataandmonitoring/Poliothisweek.aspx`). There
...
[^1]: This example is based on a newspaper report of a real poll for
    which the percentages were reported only as 47, 27, and 26 out of
    “about 700” respondents. The exact numbers used here for
    illustration have been made up to correspond to these real results.

[^2]: The Pennsylvania Supreme Court Committee on racial and gender bias
...
[^3]: The data used here are from the official evaluation of the trials
...
[^4]: Javeline, D. (1999). “Response effects in polite cultures: A test
    of acquiescence in Kazakhstan”. *Public Opinion Quarterly*, **63**,
    1–28.

[^5]: In this context the letter does *not* refer to the mathematical
    constant $\pi=3.14159\dots$, for which the same symbol is also used.

[^6]: Note that incidence was not zero even in the vaccinated group,
    ...
    at a higher rate than implied by the reduction in vaccination alone.

<!--chapter:end:05-MY451_5.rmd-->
...
Health Survey for England 2002 (HES).[^1] One part of the survey was a
...
![(\#fig:f-normbp) Illustration of probabilities for a normal distribution in the blood
pressure example, where $\mu=74.2$ and $\sigma=11.3$. The plot shows
probabilities for the ranges of values at most 60 (“Low”), between 60
and 90 ("Mid”) and over 90 (“High”).](normbp){width="12cm"}
  ...
  : (\#tab:t-normtab) Extract from the table of right-hand tail probabilities for normal
  $Z$ scores. Here “Tail Prob.” is the probability that a value from the
  standard normal distribution is at least the value in the column
  labelled “$z$”. The full table is shown in Section
  \@ref(s-disttables-Z).
...
given in Section \@ref(s-disttables-Z) in the Appendix, and an extract
...
is the kind of quantity we calculated above[^2]. The required
...
different kind of text book[^3], and because it pleases the author of
...
-   If $Y_{1}, Y_{2}, \dots, Y_{n}$ are a random sample of observations
    from (almost[^4]) any distribution with a population mean $\mu$ and
...
sizes $n$ are 10, 30, 100 and 1000.](sampld_p){width="140mm"}
...
[^1]: Carried out on behalf of The Department of Health by SCPR and the
    Department of Epidemiology and Public Health, UCL. Data used here
    were obtained from the UK Data Archive at `www.data-archive.ac.uk`.

[^2]: Note that there we were looking for the probability of a Z score
...
[^3]: Ferguson, T. S. (1996). *A Course in Large Sample Theory*, Chapman
    & Hall, London.

[^4]: The CLT does not hold in some rather weird cases which need not
    concern us here. Condition (\@ref(eq:lindeberg)) is a mathematical
    expression for “not weird”.

<!--chapter:end:06-MY451_6.rmd-->
...
(\@ref(eq:ciD1)).[^11] This is given as $(-2.30; -2.10)$. To obtain the more
...
characteristics of the countries of the world[^1]. The following
...
![(\#fig:f-corruption1) A scatterplot of Control of corruption vs. GDP per capita in the
Global Civil Society data set, for 61 countries with Control of
corruption at least 60. The dotted lines are drawn to the point
corresponding to the United Kingdom.](corruption1){width="14cm"}
...
by the NASA Goddard Institute for Space Studies[^2]. The database
...
![(\#fig:f-temperatures) Changes of average annual temperature (11-year moving averages) from
1903 in five locations. See the text for further details. Source: The GISTEMP database <data.giss.nasa.gov/gistemp/>](temperplot){width="13cm"}
...
![(\#fig:f-corruption2) The same scatterplot of Control of corruption vs. GDP per capita as
in Figure \@ref(fig:f-corruption1), augmented by the best-fitting (least
squares) straight line (solid line) and reference lines for two example
values of Control of corruption (dotted lines).](corruption2){width="13.5cm"}
...
conditional distribution of $Y$ given $X=95$[^4].
...
increases when we “increase” $X$[^5]. This is the sense in which we will
...
![(\#fig:f-scatterplots) Scatterplots of artificial data sets of two variables. Each plot also
shows the best-fitting (least squares) straight line and the correlation
coefficient $r$.](olspl1 "fig:"){width="8cm"} ![](olspl2){width="8cm"}\
![](olspl3){width="8cm"} ![](olspl4){width="8cm"}\
![](olspl5){width="8cm"} ![](olspl6){width="8cm"}\
...
![(\#fig:f-corruption3) A scatterplot of Control of corruption vs. GDP per capita for 163
countries in the Global Civil Society data set. The solid line is the
best-fitting (least squares) straight line for the points.](corruption3){width="13.5cm"}
...
linear or nonlinear associations[^6].
...
the word and the statistic are really due to Sir Francis Galton[^7].
...
![(\#fig:f-imr1) A scatterplot of net primary school enrolment ratio vs. Infant
mortality rate for countries in the Global Civil Society data set
($n=111$). The solid line is the best-fitting (least squares) straight
line for the points.](imr1){width="13.5cm"}
...
-   **Model**, because this is a statistical model in the sense
    ...
    the well-known statement by the statistician George Box[^10]:
...
![(\#fig:f-linmod-params) Illustration of the interpretation of the regression coefficients of a
simple linear regression model.](lmparams){width="12.5cm"}
...
![(\#fig:f-residuals) Illustration of the quantities involved in the definitions of least
squares estimates and the coefficient of determination $R^{2}$. See the
text for explanation.](lmresids){width="13.5cm"}
<!--
$Y_{i}-\hat{Y}_{i}$
...
reasoning used to obtain them is the **method of least squares**[^11].
...
\label{eq:H0betasimple}\end{equation} Recall that when $\beta$ is 0, there is no linear
association between the explanatory variable $X$ and the response
...
\label{eq:H0betasimple2}\end{equation} Tests of this are usually carried out against a
two-sided alternative hypothesis $H_{a}: \; \beta\ne 0$, and we will
also concentrate on this case.
...
denote $\hat{\text{se}}(\hat{\beta})$.[^12] It is calculated as
...
question[^13]. Suppose we are considering two variables $X$ and $Y$, and
...
![(\#fig:f-xyzspurious) A graphical representation of a
spurious association between $X$ and $Y$, explained by dependence on a
common cause $Z$.](xyzspurious){width="3.7cm"}
...
reports.
  ...
  variable                                                                        Coefficient      error     $t$   $P$-value      interval
...
-   **Estimates of the regression coefficients** are here denoted with
    ...
    for the estimates become difficult to write down[^14] and
...
coefficients of the dummy variables.
  ...
  --------------------------------------------------------------------- ------------- ------- --------- ----------- ----------------
  Explanatory variable                                                              \    Std.         \           \    95 % Conf. 
                                                                          Coefficient   error       $t$   $P$-value     interval
...
Krueger.[^15] Most of the choices of analyses and variables considered
...
Table \@ref(tab:t-fastfood-descr).
  ...
  Group                                         %   $(n)$                                      Affected    Wage gap      Response
                                                                                                % ($n$)   (mean for      variable:
                                                                                                           affected      Employment
                                                                                                         restaurants)     change
                                                                                                                          (mean)
...
-   The last two columns of the “**ANOVA**” (Analysis of Variance) table
    ...
    $F$-test[^16]. The null hypothesis for this is that the regression
...
[^1]: Anheier, H., Glasius, M. and Kaldor, M. (eds.) (2005). *Global
    Civil Society 2004/5*. London: Sage. The book gives detailed
    references to the indices considered here. Many thanks to Sally
    Stares for providing the data in an electronic form.

[^2]: Accessible at `data.giss.nasa.gov/gistemp/`. The temperatures used
    here are those listed in the data base under “after combining
    sources at same location”.

[^3]: More specifically, the differences are between 11-year *moving
...
[^4]: This discussion is obviously rather approximate. Strictly
...
[^5]: This wording is commonly used for convenience even in cases where
    the nature of $X$ is such that its values can never actually be
    manipulated.

[^6]: In this particular example, a more closely linear association is
...
[^7]: Galton, F. (1888). “Co-relations and their measurement, chiefly
    from anthropometric data”. *Proceedings of the Royal Society of
    London*, **45**, 135–145.

[^8]: This is slightly misleading: what actually matters in general is
    that the conditional mean is a linear function of the *parameters*
    $\alpha$ and $\beta$. This need not concern us at this stage.

[^9]: Galton, F. (1886). “Regression towards mediocrity in hereditary
...
[^10]: This exact phrase apparently first appears in Box, G.E.P. (1979).
...
[^11]: This is another old idea. Different approaches to the problem of
...
[^12]: It would have been more consistent with related notation used in
    Chapter \@ref(c-means) to denote it something like
    $\hat{\sigma}_{\hat{\beta}}$, but this would later become somewhat
    cumbersome.

[^13]: Here adapted from a discussion in Agresti and Finlay,
    *Statistical Methods for the Social Sciences* (1997).

[^14]: At least until we adopt extended, so-called matrix notation. In
    this, the least squares estimates are expressible simply as
    $\hat{\boldsymbol{\beta}}=
    (\mathbf{X}'\mathbf{X})^{-1}(\mathbf{X}'\mathbf{Y})$.

[^15]: Card, D. and Krueger, A. B. (1994). Minimum wages and employment:
    A case study of the fast-food industry in New Jersey and
    Pennsylvania. *The American Economic Review* **84**, 772–793.

[^16]: The sampling distribution of this test is the $F$ distribution.
    The letter in both refers to Sir Ronald Fisher, the founder of
    modern statistics.

<!--chapter:end:08-MY451_8.rmd-->
...
instructions for individual computer classes, which begin in Section \@ref(ss-class1).
...
### WEEK 2 class: Descriptive statistics for categorical data, and entering data {#ss-class1}
