# Samples and populations {#c_samples}

## Introduction {#s_samples_intro}
...
Section \@ref(ss_intro_def-descr), the purpose of inference is to draw
...
the connections between them. In Section \@ref(s_samples-popdistrs) we
...
continue in Chapters \@ref(c_tables}–\ref{c-means) where we gradually
introduce the basic elements of inference in the contexts of different
types of analyses.

## Finite populations {#s_samples_finpops}
...
(ESS) which we used in Chapter \@ref(c-descr1) (see page
\pageref{p_ess_example})[^1]. The ESS is a cross-national survey carried
...
## Samples from finite populations {#s_samples_samples}
...
## Conceptual and infinite populations {#s_samples_infpops}
...
probability samples discussed Sections \@ref(s_samples-finpops) and
\@ref(s_samples-samples). In the experiment, the participants were a
...
subjects defined in Section \@ref(s_samples-finpops). If you find the
...
## Population distributions {#s_samples_popdistrs}
...
\pageref{p_ess_example}. The two variables there are a person’s sex and
...
Table \@ref(t-attitude) on page \pageref{t_attitude}.
  ...
  : *``The government should take measures to reduce differences in income levels''*: Attitude towards income redistribution by sex, in a hypothetical
  ...
  and overall percentages \[in square brackets\].<span
  data-label="t_sex_attitude_pop"></span>
...
\@ref(t_sex_attitude-pop). The frequencies in this table desribe the
...
distributions in terms of proportions. Table \@ref(t_sex_attitude-pop)
...
\@ref(t_sex_attitude-pop), but the language of probabilities is more
appropriate in other cases. We can then say that Table
\@ref(t_sex_attitude-pop) shows two sets of **conditional probabilities**
...
\@ref(t_sex_attitude-pop). First, it applies also for continuous
...
## Need for statistical inference {#s_samples_inference}
