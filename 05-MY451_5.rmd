# Inference for population proportions {#c_probs}

## Introduction {#s_probs_intro}
...
\@ref(s_probs-examples). In the first two of them we consider a binary
...
Section \@ref(ss_intro_def-assoc), which distinguishes between the
...
groups are to be compared. Table \@ref(t-probex) shows basic sample
...
the two groups are reported in Table \@ref(t-probex). The proportion is
...
## Probability distribution of a dichotomous variable {#s_probs_distribution}
...
## Point estimation of a population probability {#s_probs_pointest}
...
other examples are shown in Table \@ref(t-probex), in the two-sample
examples 5.3 and 5.4 separately for the two groups.

When $Y$ is coded with values 0 and 1, $\hat{\pi}$ is also equal to the
sample mean of $Y$, since $$\bar{Y}=\frac{Y_{1}+Y_{2}+\dots+Y_{n}}{n}=
\frac{0+0+\dots+0+\overbrace{1+1+\dots+1}^{m \text{ ones}}}{n}=
\frac{m}{n}=\hat{\pi}.
\label{pihat_as_ybar}$$

## Significance test of a single proportion {#s_probs_test1sample}
...
form $$H_{0}:\; \pi=\pi_{0}
\label{H0p}$$ where $\pi_{0}$ is a given number which is either of
...
An alternative but equivalent form of (\@ref(H0p)) is expressed in terms
of the difference $$\Delta=\pi-\pi_{0}
\label{Dp}$$ ($\Delta$ is the upper-case Greek letter “Delta"). Then
(@\ref(H0p)) can also be written as $$H_{0}: \; \Delta=0,
\label{H0p2}$$ i.e. that there is no difference between the true
...
\\
H_{a}:&&  \Delta < 0.
\label{Haonelt}\end{aligned}$$ Such a hypothesis is only interested in
values of $\Delta$ to one side of 0, either larger or smaller than it.
For example, hypothesis (\@ref(Haonegt)) in the referendum example 5.1,
...
example 5.2, with $\pi_{0}=0.124$, (\@ref(Haonelt)) is the hypothesis
...
discussed further in Section \@ref(ss_means_tests3-errors)).

\label{p_onesidednull} The two-sided alternative hypothesis
(\@ref(Hatwo)) is clearly the logical opposite of the null hypothesis
(\@ref(H0p2)): if $\Delta$ is not equal to 0, it must be “not equal” to
0. So a two-sided alternative hypothesis must correspond to a “point”
null hypothesis (\@ref(H0p2)). For a one-sided alternative hypothesis,
...
### The test statistic {#ss_probs_test1sample_teststatistic}
...
\label{ttest_gen}$$ The statistic is introduced first in this form in
...
form (\@ref(H0p2)) about population differences of some kind, and each
can be tested with the test statistic (\@ref(ttest-gen)). For example,
all of the test statistics discussed in Chapters \@ref(c-probs),
\@ref(c_means) and \@ref(c-regression) of this course pack will be of this
...
In (\@ref(ttest-gen)), $\hat{\Delta}$ denotes a sample estimate of
$\Delta$. For a test of a single proportion, this is
$$\hat{\Delta} = \hat{\pi}-\pi_{0},
\label{Dhatp}$$ i.e. the difference between the sample proportion and
$\pi_{0}$. This is the core of the test statistic. Although the forms of
the two statistics seem rather different, (\@ref(Dhatp)) contains the
comparison of the observed and expected sample values that was also at
the heart of the $\chi^{2}$ test statistic (\@ref(chi2)) in Chapter
...
null hypothesis (\@ref(H0p2)) is 0, its observed value is $\hat{\Delta}$,
...
The $\hat{\sigma}_{\hat{\Delta}}$ in (\@ref(ttest-gen)) denotes an
...
of $\hat{\Delta}$. For the test statistic (\@ref(ttest-gen)), it is
...
\@ref(s_contd-clt). Its role in the test statistic is to provide an
...
For a test of the hypothesis (\@ref(H0p)) about a single proportion, the
estimated standard error under the null hypothesis is
$$\hat{\sigma}_{\hat{\Delta}} = \sqrt{\frac{\pi_{0}(1-\pi_{0})}{n}},
\label{seDhatp}$$ and the specific formula of the test statistic
(\@ref(ttest-gen)) is then $$z=\frac{\hat{\pi}-\pi_{0}}
{\sqrt{\pi_{0}(1-\pi_{0})/n}}.
\label{ztestp}$$ This is the **one-sample $z$-test statistic for a
...
formula (\@ref(ztestp)) is very simple, we can easily calculate it with a
...
\label{p_thumbp} Like the $\chi^{2}$ test of Chapter \@ref(c-tables), the
...
defined by (\@ref(ztestp)) is approximately the **standard normal
distribution**. The probability curve of this distribution is shown in
Figure \@ref(f_pval-prob). For now we just take it as given, and postpone
a general discussion of the normal distribution until Chapter
\@ref(c-means).

![ Illustration of the calculation of $P$-values from the standard
normal distribution. Here the value of the $z$-test statistic is
$z=-1.59$ (as in the referendum example 5.1). The areas in grey indicate
the two-sided $P$-values, i.e. the probabilities of values at least as
far from 0 as the observed value of $z$. <span
data-label="f_pval_prob"></span>](pval_zp){width="10cm"}

The $P$-value of the test is calculated from this distribution using the
general principles introduced in Section \@ref(ss_tables_chi2test-Pval).
...
\@ref(f_pval-prob). The curve in the plot is that of the standard normal
...
at least 16.71. These areas are not shown in Figure \@ref(f_pval-prob)
...
\label{p_onesided} Consider now the case of a one-sided alternative
...
in Figure \@ref(f_pval-prob), and the corresponding area on the right
...
\@ref(f_pval-prob), these values would correspond to the area under the
...
the $\chi^{2}$ test in Section \@ref(ss_tables_chi2test-Pval).

The first part of Table \@ref(t-ttable) shows a table of critical values
for the standard normal distribution. These values are also shown on
page \pageref{s_disttables_t} at the end of this course pack, on the
last row of a larger table (the other parts of this table will be
explained later, in Section \@ref(ss_means_inference-variants)). A
...
books.
  ...
  : A table of critical values for the standard normal distribution. The
  upper part of the table shows the critical values in one row, as in
  standard statistical tables (see the last row of the table on page
  \pageref{s_disttables_t}). The lower part of the table includes the
  same numbers rearranged to show critical values for conventional
  significance levels for one- and two-sided tests. <span
  data-label="t_ttable"></span>
...
The columns of the first part of Table \@ref(t-ttable) are labelled
...
the lower part of Table \@ref(t-ttable), slightly rearranged for clarity
...
alternative hypothesis) shown in the lower part of Table \@ref(t-ttable),
...
$P$-values is illustrated for Example 5.1 by Figure \@ref(f_pval-prob) on
...
### Conclusions from the test {#ss_probs_test1sample_conclusions}

The general principles of drawing and stating conclusions from a
significance test have already been explained in Section
\@ref(ss_tables_chi2test-conclusions), so they need not be repeated here.
...
### Summary of the test {#ss_probs_test1sample_summary}
...
## Confidence interval for a single proportion {#s_probs_1sampleci}

### Introduction {#s_probs_1sampleci_intro}
...
Section \@ref(s_probs-test1sample) we asked such a question about the
...
### Calculation of the interval {#s_probs_1sampleci_calc}
...
Section \@ref(ss_probs_test1sample-samplingd) are again satisfied.

Consider now Figure \@ref(f_pval_prob} on \pageref{f_pval-prob). One of
...
\label{cim0}$$ This is the **95 % confidence interval** for the
population difference $\Delta$. It is usually written more concisely as
$$\hat{\Delta}
\pm 1.96\, \hat{\sigma}_{\hat{\Delta}}
\label{cim1}$$ where the “plusminus” symbol $\pm$ indicates that we
calculate the two endpoints of the interval as in (\@ref(cim0)), one
...
$\hat{\sigma}_{\hat{\Delta}}$ is analogous to (\@ref(seDhatp)) used for
...
standard error $$\hat{\sigma}_{\hat{\Delta}} = \sqrt{
\frac{\hat{\pi}(1-\hat{\pi})}{n}
}
\label{sephat2}$$ and thus the 95% confidence interval
...
\sqrt{
\frac{\hat{\pi}(1-\hat{\pi})}{n}}.
\label{cip2}$$ This is typically the most useful interval for use in
...
### Interpretation of confidence intervals {#s_probs_1sampleci_int}
...
-   If we obtained many samples from the population and calculated the
    ...
    (\@ref(cip2)), approximately 95% of these intervals would contain the
...
In place of the 1.96 in (\@ref(cim1)), we may also use other numbers. To
allow for this in the notation, we can also write
$$\hat{\Delta} \pm z_{\alpha/2}\; \hat{\sigma}_{\hat{\Delta}}.
\label{ci_D_gen}$$ where the multiplier $z_{\alpha/2}$ is a number which
...
\@ref(ss_means_inference-variants)). The second is the **confidence
...
also shown in Table \@ref(t-ciq).
  ...
  : Multipliers $z_{\alpha/2}$ used to obtain confidence intervals based
  on the normal distribution, for three standard confidence levels.
  These values are substituted for $z_{\alpha/2}$ in formula
  (\@ref(ci_D-gen)) to obtain the confidence interval.<span
  data-label="t_ciq"></span>
...
The width of the interval (\@ref(ci_D-gen)) is
...
-   The standard error $\hat{\sigma}_{\hat{\Delta}}$, which in the case
    of a single proportion is (\@ref(sephat2)). This in turn depends on
...
\label{p_civstest} These results also illustrate how confidence
...
## Inference for comparing two proportions {#s_probs_2samples}
...
$$\Delta=\pi_{2}-\pi_{1}.
\label{Dp2sample}$$ For a significance test of this, the null hypothesis
will again be $H_{0}:\; \Delta=0$, which is in this case equivalent to
the hypothesis of equal proportions $$H_{0}:\; \pi_{1} = \pi_{2}.
\label{H0pD}$$ The null hypothesis thus claims that there is no
...
$\Delta$ is then $$\hat{\Delta}=\hat{\pi}_{2} - \hat{\pi}_{1}.
\label{Dhatpi}$$ This gives $\hat{\Delta}=0.000284-0.000706=-0.000422$
...
The standard error of $\hat{\Delta}$ is $$\sigma_{\hat{\Delta}} =
\sqrt{
\frac{\pi_{2}(1-\pi_{2})}{n_{2}}
+\frac{\pi_{1}(1-\pi_{1})}{n_{1}}
}.
\label{sigmaDpi}$$ As in the one-sample case above, the best way to
...
assumption that the null hypothesis (\@ref(H0pD)) is true, in which case
...
\label{phat2sample}$$ Using this for both $\pi_{1}$ and $\pi_{2}$ in
(\@ref(sigmaDpi)) gives the estimated standard error
...
\label{seDpi}$$ and using (\@ref(Dhatpi}) and (\ref{seDpi)) in the
general formula (\@ref(ttest-gen)) gives the **two-sample $z$-test
statistic for proportions** $$z=
\frac{\hat{\pi}_{2}-\hat{\pi}_{1}}
{\sqrt{\hat{\pi}(1-\hat{\pi})(1/n_{2}+1/n_{1})}}
\label{ztestDpi}$$ where $\hat{\pi}$ is given by (\@ref(phat2sample)).
...
error cannot assume that (\@ref(H0pD)) is true. Instead, we use the
estimate $$\hat{\sigma}_{\hat{\Delta}} =
\sqrt{
\frac{\hat{\pi}_{2}(1-\hat{\pi}_{2})}{n_{2}} +
\frac{\hat{\pi}_{1}(1-\hat{\pi}_{1})}{n_{1}}
}
\label{seDpi2}$$ and, substituting this to the general formula
(\@ref(ci_D-gen)), we get
...
\label{ciDpi}$$ as the confidence interval for $\Delta=\pi_{2}-\pi_{1}$,
...
\frac{257+338}{921+929} = 0.322.$$ The standard error from (\@ref(seDpi))
...
\sqrt{
\frac{0.2182}{462.5}
}=0.0217,$$ and the test statistic (\@ref(ztestDpi)) is
$$z=\frac{0.085}{0.0217}=3.92.$$ For the confidence interval, the
standard error of $\hat{\Delta}$ is estimated from (\@ref(seDpi2)) as
$$\begin{aligned}
\hat{\sigma}_{\hat{\Delta}} &=&
\sqrt{
...
\frac{0.2315}{929}
+\frac{0.2012}{921}
}=0.0216\end{aligned}$$ and a 95% confidence interval from (\@ref(ciDpi))
...
(see Table \@ref(t_probex) on page \pageref{t-probex}), i.e. 422 fewer
...
the Salk vaccine worked (see Figure \@ref(f-nytimes)), and it (and later
...
![Public reaction to statistical inference.<span
data-label="f_nytimes"></span>](salk_nytimes){width="150mm"}
