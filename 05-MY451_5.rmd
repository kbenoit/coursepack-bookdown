...
Figure \@ref(f-pval-prob). For now we just take it as given, and postpone
...
data-label="fig:f-pval_prob"></span>](pval_zp){width="10cm"}
...
\@ref(f-pval-prob). The curve in the plot is that of the standard normal
...
at least 16.71. These areas are not shown in Figure \@ref(f-pval-prob)
...
in Figure \@ref(f-pval-prob), and the corresponding area on the right
...
\@ref(f-pval-prob), these values would correspond to the area under the
...
$P$-values is illustrated for Example 5.1 by Figure \@ref(f-pval-prob) on
...
Consider now Figure \@ref(f-pval-prob} on \pageref{f-pval-prob). One of
...
also shown in Table \@ref(t-ciq).
  ...
  (\@ref(ci-D-gen)) to obtain the confidence interval.<span
  data-label="tab:t-ciq"></span>
...
The width of the interval (\@ref(ci-D-gen)) is
...
(see Table \@ref(t-probex) on page \pageref{t-probex}), i.e. 422 fewer
...
data-label="fig:f-nytimes"></span>](salk_nytimes){width="150mm"}
