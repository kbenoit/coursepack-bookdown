...
(@\ref(H0p)) can also be written as \begin{equation}H_{0}: \; \Delta=0,
...
\\
H_{a}:&&  \Delta < 0.
\label{eq:Haonelt}\end{aligned} Such a hypothesis is only interested in
...
(\@ref(ttest-gen)) is then \begin{equation}z=\frac{\hat{\pi}-\pi_{0}}
...
also shown in Table \@ref(tab:t-ciq).
  ...
  (\@ref(ci-D-gen)) to obtain the confidence interval.
...
\label{eq:seDpi}\end{equation} and using (\@ref(Dhatpi}) and (\ref{seDpi)) in the
...
\label{eq:ztestDpi}\end{equation} where $\hat{\pi}$ is given by (\@ref(phat2sample)).
