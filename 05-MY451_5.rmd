...
\ref{s_probs_examples}. In the first two of them we consider a binary
...
Section \ref{ss_intro_def_assoc}, which distinguishes between the
...
groups are to be compared. Table \ref{t_probex} shows basic sample
...
possible census error in it).
  ...
  \hline\multicolumn{8}{|l|}{**Two Independent samples**}                                                                                               
...
the two groups are reported in Table \ref{t_probex}. The proportion is
...
analyses of Chapter \ref{c_tables} we considered conditional
...
will be introduced in Chapter \ref{c_means}, where we consider a
...
other examples are shown in Table \ref{t_probex}, in the two-sample
...
Chapter \ref{c_means}. In all of these cases the quantities of interest
...
\ref{c_tables}. For the difference $\Delta$, we may consider two basic
...
discussed further in Section \ref{ss_means_tests3_errors}).
...
can be tested with the test statistic (\ref{ttest_gen}). For example,
all of the test statistics discussed in Chapters \ref{c_probs},
\ref{c_means} and \ref{c_regression} of this course pack will be of this
type (but the $\chi^{2}$ test statistic of Chapter \ref{c_tables} is
...
\ref{c_tables}. Here the “observed value” is the sample estimate
...
of $\hat{\Delta}$. For the test statistic (\ref{ttest_gen}), it is
...
(\ref{ttest_gen}) is then $$z=\frac{\hat{\pi}-\pi_{0}}
...
\label{p_thumbp} Like the $\chi^{2}$ test of Chapter \ref{c_tables}, the
...
Figure \ref{f_pval_prob}. For now we just take it as given, and postpone
...
at least 16.71. These areas are not shown in Figure \ref{f_pval_prob}
...
explained later, in Section \ref{ss_means_inference_variants}). A
...
books.
  ...
  ---------------- --------------------------------------------------- ------- ------- ------- ------- ------- --------
                     \multicolumn{7}{|c|}{Right-hand tail probability}                                         
  \cline{2-8}                                                    0.100   0.050   0.025    0.01   0.005   0.001   0.0005
  ...
  ------------------------------------ ------------------------------------------ ------ ------ -------
                                         \multicolumn{4}{|c|}{Significance level}               
  \cline{2-5} Alternative hypothesis                                         0.10   0.05   0.01   0.001
...
alternative hypothesis) shown in the lower part of Table \ref{t_ttable},
...
Section \ref{s_probs_test1sample} we asked such a question about the
...
\ref{ss_means_inference_variants}). The second is the **confidence
...
also shown in Table \ref{t_ciq}.
  ...
  (\ref{ci_D_gen}) to obtain the confidence interval.<span
...
general formula (\ref{ttest_gen}) gives the **two-sample $z$-test
...
(see Table \ref{t_probex} on page \pageref{t_probex}), i.e. 422 fewer
...
the Salk vaccine worked (see Figure \ref{f_nytimes}), and it (and later
