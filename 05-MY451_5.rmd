...
form $$H_{0}:\; \pi=\pi_{0}
\label{eq:H0p}$$ where $\pi_{0}$ is a given number which is either of
...
\label{eq:Dp}$$ ($\Delta$ is the upper-case Greek letter “Delta"). Then
(@\ref(H0p)) can also be written as $$H_{0}: \; \Delta=0,
\label{eq:H0p2}$$ i.e. that there is no difference between the true
...
\label{eq:ttest_gen}$$ The statistic is introduced first in this form in
...
\label{eq:Dhatp}$$ i.e. the difference between the sample proportion and
...
\label{eq:seDhatp}$$ and the specific formula of the test statistic
(\@ref(ttest-gen)) is then $$z=\frac{\hat{\pi}-\pi_{0}}
{\sqrt{\pi_{0}(1-\pi_{0})/n}}.
\label{eq:ztestp}$$ This is the **one-sample $z$-test statistic for a
...
\label{eq:cim0}$$ This is the **95 % confidence interval** for the
population difference $\Delta$. It is usually written more concisely as
$$\hat{\Delta}
\pm 1.96\, \hat{\sigma}_{\hat{\Delta}}
\label{eq:cim1}$$ where the “plusminus” symbol $\pm$ indicates that we
...
\label{eq:sephat2}$$ and thus the 95% confidence interval
...
\sqrt{
\frac{\hat{\pi}(1-\hat{\pi})}{n}}.
\label{eq:cip2}$$ This is typically the most useful interval for use in
...
\label{eq:ci_D_gen}$$ where the multiplier $z_{\alpha/2}$ is a number which
...
$$\Delta=\pi_{2}-\pi_{1}.
\label{eq:Dp2sample}$$ For a significance test of this, the null hypothesis
will again be $H_{0}:\; \Delta=0$, which is in this case equivalent to
the hypothesis of equal proportions $$H_{0}:\; \pi_{1} = \pi_{2}.
\label{eq:H0pD}$$ The null hypothesis thus claims that there is no
...
\label{eq:Dhatpi}$$ This gives $\hat{\Delta}=0.000284-0.000706=-0.000422$
...
\label{eq:sigmaDpi}$$ As in the one-sample case above, the best way to
...
\label{eq:phat2sample}$$ Using this for both $\pi_{1}$ and $\pi_{2}$ in
...
\label{eq:seDpi}$$ and using (\@ref(Dhatpi}) and (\ref{seDpi)) in the
...
\label{eq:ztestDpi}$$ where $\hat{\pi}$ is given by (\@ref(phat2sample)).
...
\label{eq:seDpi2}$$ and, substituting this to the general formula
(\@ref(ci-D-gen)), we get
...
\label{eq:ciDpi}$$ as the confidence interval for $\Delta=\pi_{2}-\pi_{1}$,
