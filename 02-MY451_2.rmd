# Descriptive statistics {#c_descr1}

## Introduction {#s_descr1_intro}
...
-   Methods that are used only for variables with small numbers of
    ...
    many values (see Section \@ref(sss-intro-def-vars-rels) for more on
...
variables and Section \@ref(s_descr1-2cat) bivariate methods for cases
where both variables are categorical. Sections \@ref(s_descr1-1cont) and
\@ref(s_descr1-nums) cover univariate methods which are mostly used for
continuous variables. Section \@ref(s_descr1-2cont) lists some bivariate
...
graphs in Section \@ref(s_descr1-presentation).

## Example data sets {#s_descr1_examples}
...
chapter (Section \@ref(s_descr1-app)), together with the whole data set,
shown in Table \@ref(t-countrydata).
...
\@ref(s_descr1-2cat) below we use descriptive statistics to examine such
associations between sex and attitude in this sample.

## Single categorical variable {#s_descr1_1cat}

### Describing the sample distribution {#ss_descr1_1cat_distr}
...
### Tabular methods: Tables of frequencies {#ss_descr1_1cat_tables}
...
\@ref(t_region), \@ref(t_democ) and \@ref(t-attitude) respectively.
  ...
  : Frequency distribution of the region variable in the country
  data.<span data-label="t_region"></span>
  ...
  : Frequency distribution of the democracy index in the country
  data.
  ...
  : Frequency distribution of responses to a question on attitude
  towards income redistribution in the survey example.<span
  data-label="t_attitude"></span>
...
slightly different, as it is in Table \@ref(t-democ)).

### Graphical methods: Bar charts {#ss_descr1_1cat_charts}
...
Figures \@ref(f_bars_region), \@ref(f_bars-democ) and
\@ref(f_bars-attitude) show the bar charts for region, democracy index
and attitude, corresponding to the frequencies in Tables \@ref(t-region),
\@ref(t_democ) and \@ref(t-attitude). Each bar corresponds to one category
...
![Bar chart of regions in the country data.<span
data-label="f_bars_region"></span>](regions){height="9.5cm"}

![Bar chart of the democracy index in the country data.<span
data-label="f_bars_democ"></span>](democ){height="9.5cm"}

![Bar chart of the attitude variable in the survey data example.<span
data-label="f_bars_attitude"></span>](bar_attitude "fig:"){height="8cm"}
\scriptsize{
...
-   The heights of the bars may represent frequencies, proportions
    ...
    the same in each case. In Figure \@ref(f_bars-region), the units are
    frequencies, while in Figures \@ref(f_bars-democ) and
    \@ref(f_bars-attitude) they are percentages.
...
-   If the variable is ordinal, the bars must be in the natural order of
    the categories, as in Figures \@ref(f_bars-democ) and
    \@ref(f_bars-attitude). If the variable is nominal, the order
    ...
    \@ref(f_bars-region), the frequency ordering would swap Northern
...
\@ref(f_bars-democ) quite effectively conveys the information that the
...
(c.f. the data in Table \@ref(t-countrydata)). The basic idea of such
...
### Simple descriptive statistics {#ss_descr1_1cat_descriptives}
...
be discussed together for both in Section \@ref(s_descr1-nums).

## Two categorical variables {#s_descr1_2cat}

### Two-way contingency tables {#ss_descr1_2cat_tables}
...
also known as a **contingency table**. Table \@ref(t_sex-attitude) shows
...
contingency tables:
  ...
  : *``The government should take measures to reduce differences in income levels''*: Two-way table of frequencies of respondents in the survey example,
  by sex and attitude towards income redistribution. Data: European Social Survey, Round 5, 2010, UK respondents only.<span
  data-label="t_sex_attitude"></span>

-   Because a table like \@ref(t_sex-attitude) summarizes the values of
    two variables, it is known as a **two-way** contingency table.
    Similarly, the tables of single variables introduced in Section
    \@ref(ss_descr1_1cat-tables) are *one-way* tables. It is also
...
-   The variables in a contingency table may ordinal or nominal
    ...
    practice which is discussed further in Section \@ref(s_descr1-1cont).

-   The horizontal divisions of a table (e.g. the lines corresponding to
    the two sexes in Table \@ref(t_sex-attitude)) are its **rows**, and
    the vertical divisions (e.g. the survey responses in Table
    \@ref(t_sex-attitude)) are its **columns**.

-   The size of a contingency table is stated in terms of the numbers of
    its rows and columns. For example, Table \@ref(t_sex-attitude) is a
    $2\times
...
-   The intersection of a row and a column is a **cell** of the table.
    ...
    Table \@ref(t_sex-attitude) shows that there were 160 male
...
-   The row and column labelled “Total” in Table \@ref(t_sex-attitude)
    ...
    margin for attitude in Table \@ref(t_sex-attitude) are the same as
    the ones in the one-way table for this variable shown in Table
    \@ref(t-attitude). The distributions described by the margins are
...
This is illustrated in Table \@ref(t_sex_attitude-pr), which shows the
overall proportions, obtained by dividing the frequencies in Table
\@ref(t_sex-attitude) by $n=2344$. For example, out of all these
...
in the bottom right corner of the table.
  ...
  : *``The government should take measures to reduce differences in income levels''*: Two-way table of joint proportions of respondents in the survey
  example, with each combination of sex and attitude towards income
  redistribution. Data: European Social Survey, Round 5,                                                                                                                         
  2010, UK respondents only.<span data-label="t_sex_attitude_pr"></span>

### Conditional proportions {#ss_descr1_2cat_cond}
...
\@ref(t_sex_attitude-row) show these proportions calculated for each
*row* of Table \@ref(t_sex_attitude) (Table \@ref(t_sex_attitude-row) also
...
the top left-hand corner of Table \@ref(t_sex_attitude-row) is obtained
...
as the proportions in the ‘Total’ row of Table \@ref(t_sex_attitude-pr).
  ...
  of attitude given sex. Data: European Social Survey, Round 5,                                                                                                                           
  2010, UK respondents only.<span data-label="t_sex_attitude_row"></span>
...
### Conditional distributions and associations {#ss_descr1_2cat_assoc}
...
Table \@ref(t_sex_attitude-row) we calculated conditional proportions for
...
definition in Section \@ref(ss_intro_def-assoc) stated that there is an
...
### Describing an association using conditional proportions {#ss_descr1_2cat_descr}
...
\@ref(t_sex_attitude-row) the same proportion (0.156, or 15.6%) of both
...
### A measure of association for ordinal variables {#ss_descr1_2cat_gamma}
...
“Important” or “Very important”. Table \@ref(t-investors) shows the
...
age groups.
  ...
  : Importance of short-term gains: Frequencies of respondents in the investment example, by age group
  ...
  value of the $\gamma$ measure of association is $-0.377$.<span
  data-label="t_investors"></span>
...
case in Table \@ref(t-investors). The ordering of the categories can then
...
example, in Table \@ref(t-investors) the categories corresponding to the
...
in Table \@ref(t-investors). It is clear that, for example, the
...
respondents in Table \@ref(t-investors), one with values (Under 45;
...
$$\gamma=\frac{C-D}{C+D}
\label{gamma}$$ where $C$ is the total number of concordant pairs in the
table, and $D$ is the number of discordant pairs. For Table
\@ref(t-investors), the value of this is $\gamma=-0.377$.
...
of (\@ref(gamma)) is also not crucial. More important than the formula of
...
The **sign** of the statistic: It can be seen from (\@ref(gamma)) that
...
Table \@ref(t-investors) we have $\gamma=-0.377$, indicating a negative
...
of association is the ratio $$\text{PRE}= \frac{E_{1}-E_{2}}{E_{1}},
\label{PRE}$$ i.e. the improvement in predictions as a *proportion* of
...
some unfamiliar units. The smallest possible value of (\@ref(PRE)) is
...
suppose that we consider two respondents in Table \@ref(t-investors) from
...
In the example of Table \@ref(t-investors), this means predicting whether
...
\@ref(t-investors) the association is negative, so we would always
...
table. For example, should $\gamma=-0.377$ for Table \@ref(t-investors)
...
we will calculate $\gamma$ for the variables in Table \@ref(t-investors)
separately for men and women (see Table \@ref(t-investors3)). These turn
...
-   Since “high” values of a variable were defined as ones towards the
    ...
    its absolute value. For example, in Table \@ref(t-investors) we could
...
## Sample distributions of a single continuous variable {#s_descr1_1cont}

### Tabular methods {#ss_descr1_1cont_tab}
...
values would be entirely unenlightening.
  ...
  : Frequency distribution of GDP per capita in the country data.<span
  data-label="t_gdp"></span>

Instead, we can count the frequencies for some *intervals* of values.
Table \@ref(t-gdp) shows an example of this for the GDP variable. The
...
### Graphical methods {#ss_descr1_1cont_graphs}

#### Histograms {-}

![Histogram of GDP per capita in the country data, together with the
corresponding frequency polygon.<span
data-label="f_hist_gdp"></span>](gdp){width="13.5cm"}

A **histogram** is the graphical version of a frequency table for a
grouped variable, like that in Table \@ref(t-gdp). Figure
\@ref(f_hist-gdp) shows a histogram for the GDP variable (the histogram
...
what would be sensible for a table like \@ref(t-gdp). Furthermore,
intervals like those in Table \@ref(t-gdp) are not even allowed in a
...
Figure \@ref(f_hist-gdp) (less than 2.5, 2.5–less than 5.0, 5.0–less than
...
Figure \@ref(f_hist-gdp) also shows a **frequency polygon** of the GDP
...
\@ref(f-stemgdp). First, note that the values of the variable in the
...
nearest hundred dollars (as in Table \@ref(t-countrydata)) for the plot.
...
The left-hand column in \@ref(f-stemgdp) lists the stem values in the
...
Figure \@ref(f-stemgdp) on its side, and compare to Figure
\@ref(f_hist-gdp)). It has the additional advantage that it also shows
...
example, the plot shown in Figure \@ref(f-stemhours). The variable here
...
Figure \@ref(f-stemgdp) is still quite readable and fits on a single
page.

![An annotated box plot of GDP per capita in the country data
($n=155$).<span
data-label="f_boxplot_gdp"></span>](box_gdp){width="15cm"}
...
the summary statistics defined in Section \@ref(s_descr1-nums). To
introduce the idea, one box plot is shown in Figure \@ref(f_boxplot-gdp).
...
## Numerical descriptive statistics {#s_descr1_nums}

\label{p_statistic} The tabular and graphical methods discussed above
...
### Measures of central tendency {#ss_descr1_nums_central}
...
\@ref(t_region) and \@ref(t-democ) show that the mode of the region
...
the smallest to the largest. For example, in Table \@ref(t-countrydata)
...
as shown in the last column of Table \@ref(t-democ). The median is then
...
$Y$ in the sample is defined as $$\bar{Y} = \frac{\sum Y_{i}}{n}.
\label{Ybar}$$ Here $n$ is again the sample size. The symbol $\Sigma$
...
the values of $i$). In other words, (\@ref(Ybar)) is a concise expression
...
number of categories (see also the discussion in Section
\@ref(sss_intro_def_vars-measlevels)). For example, the mean democracy
...
variable, as shown in Table \@ref(t_countries-sums). Its mean (\$8600) is
clearly much larger than the median (\$4700). This is due to the shape
of the distribution of GDP, as revealed by Figure \@ref(f_hist-gdp) or
even more clearly by the stem and leaf plot of Figure \@ref(f-stemgdp).
...
that of GDP in Figure \@ref(f_hist-gdp), with its long tail towards the
large values, is said to be **skewed to the right** or **positively
skewed**. A distribution shown in panel A of Figure \@ref(f-skews) is
...
panel B of Figure \@ref(f-skews) is (approximately) **symmetric**.
  ...
  : Summary statistics for the three variables in the country data. IQR=interquartile range; s.d.=standard deviation; *: inappropriate for a nominal variable; $\dagger$: if the democracy index is treated as an interval-level variable.<span
  data-label="t_countries_sums"></span>

![Examples of a negatively skewed and an approximately symmetric sample
...
($n=867,016$, respondents with hours 0 or not applicable omitted).<span
data-label="f_skews"></span>](twohists "fig:"){width="14cm"}\
<span>\footnotesizeSource of the data for panel B: Cathie Marsh Centre
for Census and Survey Research, University of Manchester,
`http://www.ccsr.ac.uk/sars/` </span>
...
### Measures of variation {#ss_descr1_nums_variation}
...
as shown in Table \@ref(t-classmarks).

\label{p_classex} Both the mean and the median of the marks are 6 in
...
can be used for such variables, but they are not described here.
  ...
  : A hypothetical examples of test marks of students in three
  classes.<span data-label="t_classmarks"></span>
...
average value in the sample. Table \@ref(t-sdex) shows the deviations for
...
that an observation is larger than the mean (e.g. $10-6=+4$).
  ...
  : Calculating the standard deviation of test marks for Class 3 in the
  class example of page \pageref{p_classex}.<span
  data-label="t_sdex"></span>
...
the second column of Table \@ref(t-sdex)), and not by coincidence. It can
...
by itself (c.f. the third column of Table \@ref(t-sdex) for an
illustration). These are used to calculate the **variance**, denoted
$s^{2}$ and defined as $$s^{2} = \frac{\sum (Y_{i}-\bar{Y})^{2}}{n-1}.
\label{samplevar}$$ This is (apart from the $n-1$ rather than $n$ as the
...
\label{sd}$$ For the class example, this is 0 for Class 1, 1.3 for Class
...
\@ref(ss_contd_popdistrs-params). The definition is not entirely
...
charts of Figure \@ref(f-classbars). The distribution for Class 2 is
...
\@ref(f_bars-democ)). This property would not be revealed by measures of
...
![Bar charts of the test marks in the class example of page
\pageref{p_classex} for Classes 2 (on the left) and 3 (on the
right).<span
data-label="f_classbars"></span>](classbars){height="8.3cm"}

## Associations which involve continuous variables {#s_descr1_2cont}
...
-   Explanatory variable is categorical and response variable
    ...
    (Section \@ref(s_means-descr)).

-   Both explanatory and response variables are continuous: Scatter
    plots and line plots (Section \@ref(ss_regression_descr-plots)).
...
## Presentation of tables and graphs {#s_descr1_presentation}
...
dimension, an example of which is shown in Figure \@ref(f-yuk). The
information presented in this graph is the same as that of Figure
\@ref(f_bars-region), i.e. frequencies of different regions. These are
...
the one in Figure \@ref(f-yuk) is always wrong and not acceptable under
...
![An example of an unacceptable graph: a bar chart with a pseudo
three-dimensional effect. The data are the same as in Figure
\@ref(f_bars-region).<span
data-label="f_yuk"></span>](threeD){height="8cm"}
...
Figure \@ref(f-houseprices) shows one (good but not in any way
...
message of Figure \@ref(f-houseprices) is immediately clear: increases in
...
![An example of an informative graph: house prices in three countries
between 1993 and 2003, indexed to 100 in 1993.<span
data-label="f_houseprices"></span>](houseprices){width="10cm"}

<span>\footnotesizeSource: *The Economist*, June 26th, 2004. The numbers
...
## Appendix: Country data {#s_descr1_app}

The data used for illustration throughout this chapter are given in
Table \@ref(t-countrydata). The variables are defined as follows:
...
-   **region** indicates the macro region where the country is located,
    ...
    (see <span>\small`unstats.un.org/unsd/methods/m49/m49.htm`</span>).
...
-   **GDP** is the country’s Gross Domestic Product per capita (in
    ...
    (<span>\small`https://www.cia.gov/library/publications/`</span>\
    <span>\small`the-world-factbook/`</span>). The figures refer to
...
[^4]: Lewellen, W. G., Lease, R. G., and Schlarbaum, G. G. (1977).
    ...
    and Table \@ref(t-investors) was obtained by combining these. The
