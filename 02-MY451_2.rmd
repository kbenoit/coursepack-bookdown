...
\@ref(fig:f-stemgdp). First, note that the values of the variable in the
...
The left-hand column in \@ref(fig:f-stemgdp) lists the stem values in the
...
Figure \@ref(fig:f-stemgdp) on its side, and compare to Figure
...
example, the plot shown in Figure \@ref(fig:f-stemhours). The variable here
...
Figure \@ref(fig:f-stemgdp) is still quite readable and fits on a single
page.
...
even more clearly by the stem and leaf plot of Figure \@ref(fig:f-stemgdp).
...
that an observation is larger than the mean (e.g. $10-6=+4$).
  ...
  class example of page \pageref{p_classex}.
...
![(\#fig:f-classbars) Bar charts of the test marks in the class example of page
