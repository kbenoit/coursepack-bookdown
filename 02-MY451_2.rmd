...
\@ref(t-region), \@ref(t-democ) and \@ref(t-attitude) respectively.
  ...
  data.<span data-label="tab:t-region"></span>
  ...
  data-label="tab:t-attitude"></span>
...
Figures \@ref(f-bars-region), \@ref(f-bars-democ) and
\@ref(f-bars-attitude) show the bar charts for region, democracy index
and attitude, corresponding to the frequencies in Tables \@ref(t-region),
\@ref(t-democ) and \@ref(t-attitude). Each bar corresponds to one category
...
data-label="fig:f-bars-region"></span>](regions){height="9.5cm"}

![Bar chart of the democracy index in the country data.<span
data-label="fig:f-bars-democ"></span>](democ){height="9.5cm"}

![Bar chart of the attitude variable in the survey data example.<span
data-label="fig:f-bars-attitude"></span>](bar_attitude "fig:"){height="8cm"}
\scriptsize{
...
-   The heights of the bars may represent frequencies, proportions
    ...
    the same in each case. In Figure \@ref(f-bars-region), the units are
    frequencies, while in Figures \@ref(f-bars-democ) and
    \@ref(f-bars-attitude) they are percentages.
...
-   If the variable is ordinal, the bars must be in the natural order of
    the categories, as in Figures \@ref(f-bars-democ) and
    \@ref(f-bars-attitude). If the variable is nominal, the order
    ...
    \@ref(f-bars-region), the frequency ordering would swap Northern
...
\@ref(f-bars-democ) quite effectively conveys the information that the
...
also known as a **contingency table**. Table \@ref(t-sex-attitude) shows
...
contingency tables:
  ...
  data-label="tab:t-sex-attitude"></span>

-   Because a table like \@ref(t-sex-attitude) summarizes the values of
...
-   The horizontal divisions of a table (e.g. the lines corresponding to
    the two sexes in Table \@ref(t-sex-attitude)) are its **rows**, and
    the vertical divisions (e.g. the survey responses in Table
    \@ref(t-sex-attitude)) are its **columns**.

-   The size of a contingency table is stated in terms of the numbers of
    its rows and columns. For example, Table \@ref(t-sex-attitude) is a
    $2\times
...
-   The intersection of a row and a column is a **cell** of the table.
    ...
    Table \@ref(t-sex-attitude) shows that there were 160 male
...
-   The row and column labelled “Total” in Table \@ref(t-sex-attitude)
    ...
    margin for attitude in Table \@ref(t-sex-attitude) are the same as
...
This is illustrated in Table \@ref(t-sex-attitude-pr), which shows the
overall proportions, obtained by dividing the frequencies in Table
\@ref(t-sex-attitude) by $n=2344$. For example, out of all these
...
in the bottom right corner of the table.
  ...
  2010, UK respondents only.<span data-label="tab:t-sex-attitude-pr"></span>
...
\@ref(t-sex-attitude-row) show these proportions calculated for each
*row* of Table \@ref(t-sex-attitude) (Table \@ref(t-sex-attitude-row) also
...
the top left-hand corner of Table \@ref(t-sex-attitude-row) is obtained
...
as the proportions in the ‘Total’ row of Table \@ref(t-sex-attitude-pr).
  ...
  2010, UK respondents only.<span data-label="tab:t-sex-attitude-row"></span>
...
Table \@ref(t-sex-attitude-row) we calculated conditional proportions for
...
\@ref(t-sex-attitude-row) the same proportion (0.156, or 15.6%) of both
...
age groups.
  ...
  data-label="tab:t-investors"></span>
...
Figure \@ref(f-hist-gdp) (less than 2.5, 2.5–less than 5.0, 5.0–less than
...
Figure \@ref(f-hist-gdp) also shows a **frequency polygon** of the GDP
...
\@ref(f-hist-gdp)). It has the additional advantage that it also shows
...
($n=155$).<span
data-label="fig:f-boxplot-gdp"></span>](box_gdp){width="15cm"}
...
introduce the idea, one box plot is shown in Figure \@ref(f-boxplot-gdp).
...
\@ref(t-region) and \@ref(t-democ) show that the mode of the region
...
\@ref(sss-intro-def-vars-measlevels)). For example, the mean democracy
...
variable, as shown in Table \@ref(t-countries-sums). Its mean (\$8600) is
clearly much larger than the median (\$4700). This is due to the shape
of the distribution of GDP, as revealed by Figure \@ref(f-hist-gdp) or
...
that of GDP in Figure \@ref(f-hist-gdp), with its long tail towards the
...
panel B of Figure \@ref(f-skews) is (approximately) **symmetric**.
  ...
  data-label="tab:t-countries-sums"></span>
...
data-label="fig:f-skews"></span>](twohists "fig:"){width="14cm"}\
...
can be used for such variables, but they are not described here.
  ...
  classes.<span data-label="tab:t-classmarks"></span>
...
\@ref(f-bars-democ)). This property would not be revealed by measures of
...
right).<span
data-label="fig:f-classbars"></span>](classbars){height="8.3cm"}
...
\@ref(f-bars-region), i.e. frequencies of different regions. These are
...
\@ref(f-bars-region).<span
data-label="fig:f-yuk"></span>](threeD){height="8cm"}
...
data-label="fig:f-houseprices"></span>](houseprices){width="10cm"}
