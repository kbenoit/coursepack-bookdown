...
\ref{s_descr1_nums} cover univariate methods which are mostly used for
continuous variables. Section \ref{s_descr1_2cont} lists some bivariate
...
graphs in Section \ref{s_descr1_presentation}.
...
\ref{s_descr1_2cat} below we use descriptive statistics to examine such
...
slightly different, as it is in Table \ref{t_democ}).
...
Figures \ref{f_bars_region}, \ref{f_bars_democ} and
\ref{f_bars_attitude} show the bar charts for region, democracy index
and attitude, corresponding to the frequencies in Tables \ref{t_region},
\ref{t_democ} and \ref{t_attitude}. Each bar corresponds to one category
...
-   The heights of the bars may represent frequencies, proportions
    ...
    the same in each case. In Figure \ref{f_bars_region}, the units are
    frequencies, while in Figures \ref{f_bars_democ} and
...
-   If the variable is ordinal, the bars must be in the natural order of
    the categories, as in Figures \ref{f_bars_democ} and
    \ref{f_bars_attitude}. If the variable is nominal, the order
    ...
    \ref{f_bars_region}, the frequency ordering would swap Northern
...
(c.f. the data in Table \ref{t_countrydata}). The basic idea of such
...
also known as a **contingency table**. Table \ref{t_sex_attitude} shows
...
-   Because a table like \ref{t_sex_attitude} summarizes the values of
    two variables, it is known as a **two-way** contingency table.
    Similarly, the tables of single variables introduced in Section
    \ref{ss_descr1_1cat_tables} are *one-way* tables. It is also
...
-   The variables in a contingency table may ordinal or nominal
    ...
    practice which is discussed further in Section \ref{s_descr1_1cont}.

-   The horizontal divisions of a table (e.g. the lines corresponding to
    the two sexes in Table \ref{t_sex_attitude}) are its **rows**, and
    the vertical divisions (e.g. the survey responses in Table
    \ref{t_sex_attitude}) are its **columns**.

-   The size of a contingency table is stated in terms of the numbers of
    its rows and columns. For example, Table \ref{t_sex_attitude} is a
    $2\times
...
-   The intersection of a row and a column is a **cell** of the table.
    ...
    Table \ref{t_sex_attitude} shows that there were 160 male
...
-   The row and column labelled “Total” in Table \ref{t_sex_attitude}
    ...
    margin for attitude in Table \ref{t_sex_attitude} are the same as
    the ones in the one-way table for this variable shown in Table
    \ref{t_attitude}. The distributions described by the margins are
...
\ref{t_sex_attitude} by $n=2344$. For example, out of all these
...
*row* of Table \ref{t_sex_attitude} (Table \ref{t_sex_attitude_row} also
...
the top left-hand corner of Table \ref{t_sex_attitude_row} is obtained
...
as the proportions in the ‘Total’ row of Table \ref{t_sex_attitude_pr}.
...
Table \ref{t_sex_attitude_row} we calculated conditional proportions for
...
definition in Section \ref{ss_intro_def_assoc} stated that there is an
...
\ref{t_sex_attitude_row} the same proportion (0.156, or 15.6%) of both
...
question which we will take up in Chapters \ref{c_samples} and
\ref{c_tables}.
...
“Important” or “Very important”. Table \ref{t_investors} shows the
...
case in Table \ref{t_investors}. The ordering of the categories can then
...
example, in Table \ref{t_investors} the categories corresponding to the
...
respondents in Table \ref{t_investors}, one with values (Under 45;
...
\ref{t_investors}, the value of this is $\gamma=-0.377$.
...
Table \ref{t_investors} we have $\gamma=-0.377$, indicating a negative
...
suppose that we consider two respondents in Table \ref{t_investors} from
...
In the example of Table \ref{t_investors}, this means predicting whether
...
\ref{t_investors} the association is negative, so we would always
...
table. For example, should $\gamma=-0.377$ for Table \ref{t_investors}
...
different samples or groups. For example, in Chapter \ref{c_3waytables}
we will calculate $\gamma$ for the variables in Table \ref{t_investors}
separately for men and women (see Table \ref{t_investors3}). These turn
...
-   Since “high” values of a variable were defined as ones towards the
    ...
    its absolute value. For example, in Table \ref{t_investors} we could
...
-   $\gamma$ can also be treated as an estimate of the corresponding
    ...
    introduced in Chapter \ref{c_tables}.
...
Table \ref{t_gdp} shows an example of this for the GDP variable. The
...
grouped variable, like that in Table \ref{t_gdp}. Figure
...
Figure \ref{f_hist_gdp} (less than 2.5, 2.5–less than 5.0, 5.0–less than
...
Figure \ref{f_hist_gdp} also shows a **frequency polygon** of the GDP
...
nearest hundred dollars (as in Table \ref{t_countrydata}) for the plot.
...
example, the plot shown in Figure \ref{f_stemhours}. The variable here
...
Figure \ref{f_stemgdp} is still quite readable and fits on a single
page.
...
introduce the idea, one box plot is shown in Figure \ref{f_boxplot_gdp}.
...
\ref{t_region} and \ref{t_democ} show that the mode of the region
...
as shown in the last column of Table \ref{t_democ}. The median is then
...
variable, as shown in Table \ref{t_countries_sums}. Its mean (\$8600) is
clearly much larger than the median (\$4700). This is due to the shape
of the distribution of GDP, as revealed by Figure \ref{f_hist_gdp} or
even more clearly by the stem and leaf plot of Figure \ref{f_stemgdp}.
...
that of GDP in Figure \ref{f_hist_gdp}, with its long tail towards the
large values, is said to be **skewed to the right** or **positively
skewed**. A distribution shown in panel A of Figure \ref{f_skews} is
...
panel B of Figure \ref{f_skews} is (approximately) **symmetric**.
...
as shown in Table \ref{t_classmarks}.
...
average value in the sample. Table \ref{t_sdex} shows the deviations for
...
by itself (c.f. the third column of Table \ref{t_sdex} for an
...
\ref{ss_contd_popdistrs_params}. The definition is not entirely
...
charts of Figure \ref{f_classbars}. The distribution for Class 2 is
...
\ref{f_bars_democ}). This property would not be revealed by measures of
...
-   Explanatory variable is categorical and response variable
    ...
    (Section \ref{s_means_descr}).

-   Both explanatory and response variables are continuous: Scatter
    plots and line plots (Section \ref{ss_regression_descr_plots}).
...
message of Figure \ref{f_houseprices} is immediately clear: increases in
...
Table \ref{t_countrydata}. The variables are defined as follows:
...
[^4]: Lewellen, W. G., Lease, R. G., and Schlarbaum, G. G. (1977).
    ...
    and Table \ref{t_investors} was obtained by combining these. The
