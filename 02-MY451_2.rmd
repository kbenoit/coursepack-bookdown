...
$$\gamma=\frac{C-D}{C+D}
\label{eq:gamma}$$ where $C$ is the total number of concordant pairs in the
...
\label{eq:PRE}$$ i.e. the improvement in predictions as a *proportion* of
...
\label{eq:Ybar}$$ Here $n$ is again the sample size. The symbol $\Sigma$
...
\label{eq:samplevar}$$ This is (apart from the $n-1$ rather than $n$ as the
...
\label{eq:sd}$$ For the class example, this is 0 for Class 1, 1.3 for Class
