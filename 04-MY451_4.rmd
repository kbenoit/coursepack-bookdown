...
\@ref(t-sex-attitude-ch4), including both the frequencies and the
conditional proportions for attitude given sex.
  ...
  data-label="tab:t-sex-attitude-ch4"></span>
...
\@ref(t-sex-attitude-ch4) the conditional distributions of attitude are
...
independence for the data in Table \@ref(t-sex-attitude-ch4). Output for
...
\@ref(t-sex-attitude-ch4).<span
data-label="fig:f-spsschi2"></span>](chi2test_ess){width="100mm"}
...
\@ref(t-sex-attitude-ch4) for illustration. All of the elements of the
test statistic for this example are shown in Table
\@ref(t-sex-attitude-chi2). These elements are
...
-   The **observed frequencies**, denoted $f_{o}$, one for each cell of
    ...
    $f_{o}$ column of Table \@ref(t-sex-attitude-chi2) to the counts in
    Table \@ref(t-sex-attitude-ch4)).
...
-   The **expected frequencies** $f_{e}$, also one for each cell. These
    ...
    \@ref(t-sex-attitude-ch4) for Male respondents who strongly agree
    ...
    proportion in Table \@ref(t-sex-attitude-ch4) is $366/2344=0.156$. If
...
-   The difference $f_{o}-f_{e}$ between observed and expected
    ...
    last row of Table \@ref(t-sex-attitude-chi2)). Instead,
    we consider...
...
-   Finally, aggregating these quantities to get the $\chi^{2}$ test
  ...
  \@ref(t-sex-attitude-ch4). In the second column, SA, A, 0, D, and SD
  are abbreviations for Strongly agree, Agree, Neither agree nor
  disagree, Disagree and Strongly disagree respectively.<span
  data-label="tab:t-sex-attitude-chi2"></span>

The calculations can be done even by hand, but we will usually leave
them to a computer. The last column of Table \@ref(t-sex-attitude-chi2)
shows that for Table \@ref(t-sex-attitude-ch4) the test statistic is
...
\@ref(t-sex-attitude-ch4) we had 200 men who disagreed with the statement
...
\@ref(t-sex-attitude-ch4) was drawn would instead produce a sample which
...
part of Table \@ref(t-sex-attitude-H0pop) shows the crosstabulation of
...
*Population (frequencies are in millions of people):*
  ...
  this population.<span data-label="tab:t-sex-attitude-H0pop"></span>
...
*Sample:*
  ...
  : $\chi^{2}=2.8445$ <span data-label="tab:t-sex-attitude-H0pop"></span>
...
one in the upper part of Table \@ref(t-sex-attitude-H0pop). The lower
...
\@ref(t-sex-attitude-H0pop). The figure shows the histogram of the values
...
\@ref(t-sex-attitude-H0pop). Superimposed on the histogram is the curve
of the approximate sampling distribution, which is the $\chi^{2}$
distribution with 4 degrees of freedom. <span
data-label="fig:f-chisampld"></span>](chi2sims){width="8.5cm"}
...
data-label="fig:f-chi2dists"></span>](chi2dists){width="115mm"}
...
\@ref(t-sex-attitude-ch4), for example, has $R=2$ rows and $C=5$ columns,
...
in the lower part of Table \@ref(t-sex-attitude-H0pop), for which the
...
The test statistic for the real sample in Table \@ref(t-sex-attitude-ch4)
...
data-label="fig:f-pvalchisq"></span>](chi2_pval){width="8cm"}
...
statistic was 9.49, $P$ would be 0.050.
  ...
  distributions. Row 2-5 show the right-hand tail probability<span data-label="tab:t-chi2table"></span>
...
\@ref(t-sex-attitude-H0pop) we had $\chi^{2}=2.84$, which is smaller than
...
data in Table \@ref(t-sex-attitude-ch4), which is larger than the 18.47
...
4.  The test statistic: the $\chi^{2}$ statistic $$\chi^{2} = \sum
    ...
    over all of the cells. For Table \@ref(t-sex-attitude-ch4),
    $\chi^{2}=18.86$.
