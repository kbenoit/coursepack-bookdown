# Statistical inference for two-way tables {#c_tables}

## Introduction {#s_tables_intro}
...
\@ref(ss_descr1_2cat-tables). We will use again the example from the
...
\@ref(t_sex_attitude-ch4), including both the frequencies and the
conditional proportions for attitude given sex.
  ...
  : *``The government should take measures to reduce differences in income levels''*: Frequencies of respondents in the survey example, by sex and
  attitude towards income redistribution. The numbers in parentheses are
  conditional proportions of attitude given sex. Data: European Social Survey, Round 5, 2010, UK respondents only.<span
  data-label="t_sex_attitude_ch4"></span>

Unlike in Section \@ref(ss_descr1_2cat-tables), we will now go beyond
...
\@ref(s_tables-summary).

## Significance tests {#s_tables_tests}
...
\@ref(t_sex_attitude-ch4) the conditional distributions of attitude are
certainly not identical for men and women. According to the definition
in Section \@ref(ss_descr1_2cat-assoc), this shows that sex and attitude
...
\label{p_test_intuition} A significance test uses a statistic calculated
...
## The $\chi^{2}$ test of independence {#s_tables_chi2test}

### Hypotheses {#ss_tables_chi2test_null}
...
population}.
\label{H0_chi2}$$ In our example the null hypothesis is thus that a
person’s sex and his or her attitude toward income redistribution are
independent in the population of adults in the UK.

The null hypothesis (\@ref(H0-chi2)) and the $\chi^{2}$ test itself are
...
null hypothesis (\@ref(H0-chi2)) can also be stated as the claim that the
...
(\@ref(H0-chi2)),
i.e. $$H_{a}: \;\text{The variables are not statistically independent in the
population}.
\label{Ha_chi2}$$ In terms of conditional distributions, $H_{a}$ is that
...
### Assumptions of a significance test {#ss_tables_chi2test_ass}

In the following discussion we will sometimes refer to Figure
\@ref(f-spsschi2), which shows SPSS output for the $\chi^{2}$ test of
independence for the data in Table \@ref(t_sex_attitude-ch4). Output for
...
![SPSS output of the $\chi^{2}$ test of independence (here labelled
“Pearson Chi-square”) for the data in Table
\@ref(t_sex_attitude-ch4).<span
data-label="f_spsschi2"></span>](chi2test_ess){width="100mm"}
...
\label{p_chi2_fesize} The second common class of assumptions are
...
\@ref(f-spsschi2). This gives information on the number and proportion of
...
### The test statistic {#ss_tables_chi2test_stat}

A **test statistic** is a number calculated from the sample (i.e. a
statistic in the sense defined on page \pageref{p_statistic}) which is
...
\@ref(t_sex_attitude-ch4) for illustration. All of the elements of the
test statistic for this example are shown in Table
\@ref(t_sex_attitude-chi2). These elements are
...
-   The **observed frequencies**, denoted $f_{o}$, one for each cell of
    ...
    $f_{o}$ column of Table \@ref(t_sex_attitude-chi2) to the counts in
    Table \@ref(t_sex_attitude-ch4)).
...
-   The **expected frequencies** $f_{e}$, also one for each cell. These
    ...
    \@ref(t_sex_attitude-ch4) for Male respondents who strongly agree
    ...
    proportion in Table \@ref(t_sex_attitude-ch4) is $366/2344=0.156$. If
...
-   The difference $f_{o}-f_{e}$ between observed and expected
    ...
    last row of Table \@ref(t_sex_attitude-chi2)). Instead,
    we consider...
...
-   Finally, aggregating these quantities to get the $\chi^{2}$ test
    statistic $$\chi^{2} = \sum \frac{(f_{o}-f_{e})^{2}}{f_{e}}.
    \label{chi2}$$ Here the summation sign $\Sigma$ indicates that
  ...
  : Calculating the $\chi^{2}$ test statistic for Table
  \@ref(t_sex_attitude-ch4). In the second column, SA, A, 0, D, and SD
  are abbreviations for Strongly agree, Agree, Neither agree nor
  disagree, Disagree and Strongly disagree respectively.<span
  data-label="t_sex_attitude_chi2"></span>

The calculations can be done even by hand, but we will usually leave
them to a computer. The last column of Table \@ref(t_sex_attitude-chi2)
shows that for Table \@ref(t_sex_attitude-ch4) the test statistic is
$\chi^{2}=18.81$ (which includes some rounding error, the correct value
is 18.862). In the SPSS output in Figure \@ref(f-spsschi2), it is given
in the “Value” column of the “Pearson Chi-Square” row.

### The sampling distribution of the test statistic {#ss_tables_chi2test_sdist}
...
The $\chi^{2}$ test statistic (\@ref(chi2)) is a *statistic* as defined
defined on page \pageref{p_statistic}, that is a number calculated from
...
\@ref(t_sex_attitude-ch4) we had 200 men who disagreed with the statement
...
\@ref(t_sex_attitude-ch4) was drawn would instead produce a sample which
...
(\@ref(H0-chi2)). To make these ideas a little more concrete, the upper
part of Table \@ref(t_sex_attitude-H0pop) shows the crosstabulation of
...
*Population (frequencies are in millions of people):*
  ...
  : *``The government should take measures to reduce differences in income levels''*: Attitude towards income redistribution by sex (with row proportions
  ...
  this population.<span data-label="t_sex_attitude_H0pop"></span>
...
*Sample:*
  ...
  : $\chi^{2}=2.8445$ <span data-label="t_sex_attitude_H0pop"></span>
...
one in the upper part of Table \@ref(t_sex_attitude-H0pop). The lower
...
Figure \@ref(f-chisampld) shows what we observe if do a computer
simulation to draw many more samples from the population in Table
\@ref(t_sex_attitude-H0pop). The figure shows the histogram of the values
...
![Example of the sampling distribution of the $\chi^{2}$ test statistic
...
\@ref(t_sex_attitude-H0pop). Superimposed on the histogram is the curve
of the approximate sampling distribution, which is the $\chi^{2}$
distribution with 4 degrees of freedom. <span
data-label="f_chisampld"></span>](chi2sims){width="8.5cm"}
...
\@ref(f-chi2dists) shows the probability curves of some $\chi^{2}$
...
![Probability curves of some $\chi^{2}$ distributions with different
degrees of freedom (df).<span
data-label="f_chi2dists"></span>](chi2dists){width="115mm"}

For the $\chi^{2}$ test statistic of independence we have the following
result:

-   When the null hypothesis (\@ref(H0-chi2)) is true in the population,
    the sampling distribution of the test statistic (\@ref(chi2))
...
\@ref(t_sex_attitude-ch4), for example, has $R=2$ rows and $C=5$ columns,
so its degrees of freedom are $df=(2-1)\times(5-1)=4$ (as indicated by
the “df” column of the SPSS output of Figure \@ref(f-spsschi2)). Figure
\@ref(f-chisampld) shows the curve of the $\chi^{2}$ distribution with
...
expected frequencies that were discussed on page
...
long as the sample size is large enough for the conditions on page
\pageref{p_chi2_fesize} to be satisfied.

### The $P$-value {#ss_tables_chi2test_Pval}
...
-   \label{p_pval_ref} The $\mathbf{P}$**-value** is the probability, if
...
Figure \@ref(f-pvalchisq) illustrates this calculation. It shows the
...
in the lower part of Table \@ref(t_sex_attitude-H0pop), for which the
...
For a probability curve like the one in Figure \@ref(f-pvalchisq), areas
...
2.84, which is shown in grey in Figure \@ref(f-pvalchisq). This is
$P=0.585$.

The test statistic for the real sample in Table \@ref(t_sex_attitude-ch4)
was $\chi^{2}=18.86$, so the $P$-value is the combined probability of
this and all larger values. This is also shown in Figure
\@ref(f-pvalchisq). However, this area is not really visible in the plot
...
![Illustration of the $P$-value for a $\chi^{2}$ test statistic with 4
degrees of freedom and with values $\chi^{2}=2.84$ (area of the grey
region under the curve) and $\chi^{2}=18.86$.<span
data-label="f_pvalchisq"></span>](chi2_pval){width="8cm"}

\label{p_spss2b} In practice the $P$-value is usually calculated by a
computer. In the SPSS output of Figure \@ref(f-spsschi2) is is shown in
...
Such a table is also included on page \pageref{s_disttables_chi2} of
this coursepack. An extract from the table is shown in Table
\@ref(t-chi2table). Each row of the table corresponds to a $\chi^{2}$
...
statistic was 9.49, $P$ would be 0.050.
  ...
  : An extract from a table of critical values for $\chi^{2}$
  distributions. Row 2-5 show the right-hand tail probability<span data-label="t_chi2table"></span>
...
\@ref(t_sex_attitude-H0pop) we had $\chi^{2}=2.84$, which is smaller than
...
also Figure \@ref(f-pvalchisq)). Similarly, $\chi^{2}=18.86$ for the real
data in Table \@ref(t_sex_attitude-ch4), which is larger than the 18.47
...
### Drawing conclusions from a test {#ss_tables_chi2test_conclusions}
...
distribution in Table \@ref(t-chi2table).
...
discussed in Section \@ref(s_descr1-2cat).

## Summary of the $\chi^{2}$ test of independence {#s_tables_summary}
...
4.  The test statistic: the $\chi^{2}$ statistic $$\chi^{2} = \sum
    ...
    over all of the cells. For Table \@ref(t_sex_attitude-ch4),
    $\chi^{2}=18.86$.
...
methods for two way tables discussed in Section \@ref(s_descr1-2cat).
