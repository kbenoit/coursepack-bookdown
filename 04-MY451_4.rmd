...
\@ref(t_sex_attitude_ch4), including both the frequencies and the
...
\@ref(t_sex_attitude_ch4) the conditional distributions of attitude are
certainly not identical for men and women. According to the definition
in Section \@ref(ss_descr1_2cat_assoc), this shows that sex and attitude
...
null hypothesis (\@ref(H0_chi2)) can also be stated as the claim that the
...
\@ref(t_sex_attitude_ch4) for illustration. All of the elements of the
test statistic for this example are shown in Table
\@ref(t_sex_attitude_chi2). These elements are
...
-   The **observed frequencies**, denoted $f_{o}$, one for each cell of
    ...
    $f_{o}$ column of Table \@ref(t_sex_attitude_chi2) to the counts in
    Table \@ref(t_sex_attitude_ch4)).
...
-   The **expected frequencies** $f_{e}$, also one for each cell. These
    ...
    \@ref(t_sex_attitude_ch4) for Male respondents who strongly agree
    ...
    proportion in Table \@ref(t_sex_attitude_ch4) is $366/2344=0.156$. If
...
-   The difference $f_{o}-f_{e}$ between observed and expected
    ...
    last row of Table \@ref(t_sex_attitude_chi2)). Instead,
    we consider...
...
-   Finally, aggregating these quantities to get the $\chi^{2}$ test
  ...
  \@ref(t_sex_attitude_ch4). In the second column, SA, A, 0, D, and SD
...
them to a computer. The last column of Table \@ref(t_sex_attitude_chi2)
shows that for Table \@ref(t_sex_attitude_ch4) the test statistic is
$\chi^{2}=18.81$ (which includes some rounding error, the correct value
is 18.862). In the SPSS output in Figure \@ref(f_spsschi2), it is given
...
\@ref(c_means). In this chapter we will discuss them relatively
...
\@ref(t_sex_attitude_H0pop). Superimposed on the histogram is the curve
...
\@ref(f_chi2dists) shows the probability curves of some $\chi^{2}$
distributions (what such curves mean is explained in more detail below,
and in Chapter \@ref(c_means)). All of the distributions are skewed to
...
-   When the null hypothesis (\@ref(H0_chi2)) is true in the population,
...
computer. In the SPSS output of Figure \@ref(f_spsschi2) is is shown in
...
\@ref(t_chi2table). Each row of the table corresponds to a $\chi^{2}$
...
\@ref(t_sex_attitude_H0pop) we had $\chi^{2}=2.84$, which is smaller than
...
also Figure \@ref(f_pvalchisq)). Similarly, $\chi^{2}=18.86$ for the real
data in Table \@ref(t_sex_attitude_ch4), which is larger than the 18.47
...
distribution in Table \@ref(t_chi2table).
...
discussed in Section \@ref(s_descr1_2cat).
...
4.  The test statistic: the $\chi^{2}$ statistic $$\chi^{2} = \sum
    ...
    over all of the cells. For Table \@ref(t_sex_attitude_ch4),
    $\chi^{2}=18.86$.
...
methods for two way tables discussed in Section \@ref(s_descr1_2cat).
