# Analysis of population means {#c_means}
...
**Two independent samples**                                                          $n$      $\bar{Y}$       $s$       Diff.
...
**Two dependent samples**                                                            $n$      $\bar{Y}$       $s$       Diff.
...
: Examples of analyses of population means used in Chapter \ref{c_means}. Here $n$ and $\bar{Y}$ denote the sample size and sample mean respectively, in the two-group examples 7.2–7.5 separately for the two groups. “Diff.” denotes the between-group difference of means, and $s$ is the sample standard deviation of the response variable $Y$ for the whole sample (Example 7.1), of the response variable within each group (Examples 7.2 and 7.3), or of the within-pair differences (Examples 7.4 and 7.5).<span data-label="t_groupex"></span>
...
**Example 7.3: Eye contact and perceived friendliness of
police officers**
...
\newpage
\label{p_dependentex} **Example 7.4: Transition to
parenthood**
...
junctions.
  ...
  : Data for Example 7.5: Traffic flows between junctions of the M25 on
...
3.  The two samples are independent, in the sense discussed on page
...
two-sided alternative hypotheses $$H_{a}: \; \Delta\ne 0\label{Hatwom}$$ or one of the one-sided alternative hypotheses
...
![ Curves of two $t$ distributions with small degrees of freedom,
...
the variables are shown in Table \ref{t_ttests1}
  ...
  Alternative hypotheses $H_{a}: \mu<5$ and $\mu>35$ respectively.<span data-label="t_ttests1"></span>
...
$$H_{a}:  \mu> \mu_{0} \label{Ha1onegt}$$
\center{or}
$$H_{a}:  \mu< \mu_{0}.\label{Ha1onelt}$$ For example, we might consider the
...
\label{H0D}$$ and possible alternative hypotheses as
$$H_{0}:  \Delta\ne 0,  \label{HaDtwo}$$
$$H_{0}:  \Delta> 0 \label{HaDonegt}$$
\center{and}
$$H_{0}:  \Delta< 0,   \label{HaDonelt}$$ corresponding to
...
## Inference for dependent samples {#s_means_dependent}
...
\ref{s_means_1sample} to the observed values of $D$.
  ...
  $\dagger$ Obtained from the standard normal distribution.)<span data-label="t_2tests_dep"></span>
...
### Significance vs. importance {#ss_means_tests3_importance}
...
[^6]: In this case this is a consquence of the fact that the sample
    ...
    (\ref{seDmu_ne}) actually give exactly the same value for the
