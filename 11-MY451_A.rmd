...
1.  *VOTE\_LIBDEM* is a dichotomous variable indicating whether a
    ...
    and the relevant formulas are (\@ref(eq:ztestp))
    for the test statistic and (\@ref(eq:cip2)) for the
    confidence interval.
    ...
    -   For the one-sample significance test, the value of $\pi$ under
        ...
        (\@ref(eq:ztestp)), the value of the test statistic $z$ is thus
...
2.  The variable *TVDEBATE* indicates whether the respondent reports
    ...
    population proportions is thus (\@ref(eq:ztestDpi)), and a confidence interval for the difference of
    the porportions is (\@ref(eq:ciDpi)).
    ...
    -   Begin by calculating the relevant sample proportions. The
        ...
        proportion $\hat{\pi}$ (formula \@ref(eq:phat2sample)) used in the
        test statistic (\@ref(eq:ztestDpi)) is here $\hat{\pi}=0.260$, shown
        on the “Total” row.
