...
web-based **Moodle** system. See instructions on page
...
introductory guide books such as\label{p_spssbooks}
...
-   **Data editor**: A data set is displayed in the Data Editor window.
    ...
    -   **Data view**, which shows the data matrix in the
        ...
        \ref{ss_intro_def_subj}, with units in the rows and variables in
        the columns.
...
names**\label{p_listoptions}. This instructs SPSS to display the names
...
## WEEK 2 class: Descriptive statistics for categorical data, and entering data {#p_class1}
...
2.  **Information in the Variable View window.** The data file is now
    ...
    -   \label{p_missingcodes} **Missing** specifies *missing data
...
-   Names of the variables for which the statistics are requested are
    ...
    the way the variables are displayed in the list; see page
...
-   (If you are interested in the $\gamma$ measure of association for
    ...
    using textbf<span>Statistics/Ordinal/Gamma</span>. In the output the
...
-   Tests and confidence intervals for single means (c.f. Section
    \ref{s_means_1sample}) are not considered today. These are obtained
    ...
    *dependent* samples (c.f. Section \ref{s_means_dependent}).
...
2.  Obtain a $t$-test and confidence interval for the difference of
    ...
    **Group 2:** *FR* as coded in the data). Details of SPSS output for
    this are explained in Chapter \ref{c_means}; you can use the results
...
-   SPSS menus do not provide procedures for calculating the tests and
    ...
    \ref{c_probs}. This is not a serious limitation, as the calculations
    are quite simple.
...
1.  *VOTE\_LIBDEM* is a dichotomous variable indicating whether a
    ...
    and the relevant formulas are (\ref{ztestp}) on p. \pageref{ztestp}
    for the test statistic and (\ref{cip2}) on p. \pageref{cip2} for the
    confidence interval.
    ...
    -   Begin by creating a frequency table of *VOTE\_LIBDEM*. This
        ...
        $\hat{\pi}=0.260$ in the notation of Chapter \ref{c_probs}.
    ...
    -   The (two-sided) $P$-value for this is the probability that a
        ...
        distribution (c.f. Table \ref{t_ttable} on
        p. \pageref{t_ttable}) as explained in Section
        \ref{ss_probs_test1sample_samplingd}. Here, for example, $z$ is
...
2.  The variable *TVDEBATE* indicates whether the respondent reports
    ...
    analysis discussed in Section \ref{s_probs_2samples}. The formula of
    the $z$-test statistic for testing the hypothesis of equal
    population proportions is thus (\ref{ztestDpi}) on page
    \pageref{ztestDpi}, and a confidence interval for the difference of
...
#  Statistical tables {#c_disttables}


Explanation of the “Table of standard normal tail probabilities” on page
...
## Table of standard normal tail probabilities {#s_disttables_Z}
...
## Table of critical values for $t$-distributions {#s-disttables-t}
  ...
  \cline{2-8} df                                                 0.100   0.050    0.025    0.010    0.005     0.001    0.0005
...
## Table of critical values for $\chi^{2}$ distributions {#s_disttables_chi2}
...
[^2]: The data were obtained from
    <span>\scriptsize`http://data.london.gov.uk/datastore/package/london-borough-profiles`</span>.
    ...
    <span>\scriptsize`http://www.londoncouncils.gov.uk/londonfacts/londonlocalgovernment/londonmapandlinks/default.htm`</span>.
